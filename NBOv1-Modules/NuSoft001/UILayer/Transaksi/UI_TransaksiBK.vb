...
Friend Class UI_TransaksiBK
  ...
  Overrides Sub LoadBeforeInitialize()
    ...
    colDivisi.Visible = setting.MultiDivisi
		...
		BindingDataSource()
		If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("BKTransaksi.xml")) = True Then
			LayoutControl1.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("BKTransaksi.xml"))
		End If
		If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("BKTransaksiGrid.xml")) = True Then
      xGridView.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("BKTransaksiGrid.xml"))
    End If
  ...
  Overrides Sub SimpanData()
    ...
    Dim frm As New RF_BuktiHarian(Persistent.SumberDataCollection.GetEnum(instance.Sumber), MainClass.reportName.BuktiBankKeluar)
  ...
  End Sub
	Private Sub BindingDataSource()
      ...
      colAkunRepo.DataSource = Logic.Coa.GetCoa(session, True, New Persistent.CoaEnum.eKind() {Persistent.CoaEnum.eKind.AktivaLancarLainnya, Persistent.CoaEnum.eKind.AktivaTetap, Persistent.CoaEnum.eKind.AkumulasiPenyusutan, Persistent.CoaEnum.eKind.BiayaLainLain, Persistent.CoaEnum.eKind.BiayaOperasional, Persistent.CoaEnum.eKind.HargaPokokPenjualan, Persistent.CoaEnum.eKind.HutangJangkaPanjang, Persistent.CoaEnum.eKind.HutangJangkaPendek, Persistent.CoaEnum.eKind.HutangLancarLainnya, Persistent.CoaEnum.eKind.Modal, Persistent.CoaEnum.eKind.Pendapatan, Persistent.CoaEnum.eKind.PendapatanLainLain, Persistent.CoaEnum.eKind.Persediaan, Persistent.CoaEnum.eKind.Piutang, Persistent.CoaEnum.eKind.Bank, Persistent.CoaEnum.eKind.Kas, Persistent.CoaEnum.eKind.Pajak}).Where(Function(w) w.Aktif).ToList
    Else
      colAkunRepo.DataSource = Logic.Coa.GetCoa(session, True, New Persistent.CoaEnum.eKind() {Persistent.CoaEnum.eKind.AktivaLancarLainnya, Persistent.CoaEnum.eKind.AktivaTetap, Persistent.CoaEnum.eKind.AkumulasiPenyusutan, Persistent.CoaEnum.eKind.BiayaLainLain, Persistent.CoaEnum.eKind.BiayaOperasional, Persistent.CoaEnum.eKind.HargaPokokPenjualan, Persistent.CoaEnum.eKind.HutangJangkaPanjang, Persistent.CoaEnum.eKind.HutangJangkaPendek, Persistent.CoaEnum.eKind.HutangLancarLainnya, Persistent.CoaEnum.eKind.Modal, Persistent.CoaEnum.eKind.Pendapatan, Persistent.CoaEnum.eKind.PendapatanLainLain, Persistent.CoaEnum.eKind.Persediaan, Persistent.CoaEnum.eKind.Piutang}).Where(Function(w) w.Aktif).ToList
    End If
    ...
    txtAkun.Properties.DataSource = Logic.Coa.GetCoa(session, True, New Persistent.CoaEnum.eKind() {Persistent.CoaEnum.eKind.Bank}).Where(Function(w) w.Aktif).ToList
    txtCaraBayar.Properties.DataSource = Utils.Helper.EnumDescription.ToList(GetType(Persistent.BankCaraBayar))
		...
		End If
		'txtCopy.Properties.DataSource = New XPQuery(Of Persistent.GlMain)(session).Where(Function(w) w.Sumber Is sumber.GetObject(Persistent.SumberDataJenis.BK))
		txtCopy.Properties.DataSource = New XPCollection(Of Persistent.GlMain)(session, New BinaryOperator("Sumber", sumber.GetObject(Persistent.SumberDataJenis.BK), BinaryOperatorType.Equal))
	End Sub
    ...
    If kode <> Nothing Then
      x = "#" & kode
    End If
    Me.Text = "Bank Keluar : " & Tipe.ToString & x
  End Sub
  ...
  Private Sub LoadDefault()
    ...
    txtUraian.EditValue = setting.UraianBK
  End Sub
  ...
  Private Sub SimpanCaraBayar(item As Persistent.BankCaraBayar)
    Select Case item
      ...
      Case Persistent.BankCaraBayar.CekGiro, Persistent.BankCaraBayar.CekGiroTempo
        ...
        giro.Tipe = Persistent.eJenisGiro.Keluar
  ...
  Private Sub xGridView_ColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs) Handles xGridView.ColumnWidthChanged
    xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("BKTransaksiGrid.xml"))
  End Sub
	Private Sub btnCopy_Click(sender As Object, e As EventArgs) Handles btnCopy.Click
		...
		If Not txtCopy.EditValue Is Nothing Then
			...
			For pi = 0 To xMain.Detail.Count - 1
				If xMain.Detail(pi).Debit > 0 Then
					Dim item As Persistent.GlMainDetail
					item = New Persistent.GlMainDetail(session)
