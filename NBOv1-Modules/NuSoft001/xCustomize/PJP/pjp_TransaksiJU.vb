...
Friend Class pjp_TransaksiJU
  ...
  Overrides Sub LoadBeforeInitialize()
    ...
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("pjpJUTransaksi.xml")) = True Then
      lytTransaksiJU.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("pjpJUTransaksi.xml"))
    End If
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("pjpJUTransaksiGrid.xml")) = True Then
      xGridView.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("pjpJUTransaksiGrid.xml"))
    End If
  End Sub
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      '  instance.CashAdvance = session.GetObjectByKey(Of Persistent.xPJPGLMain)(txtCashAdvance.EditValue)
      'End If
    ...
    Dim frm As New RF_BuktiHarian(Persistent.SumberDataJenis.JU, MainClass.reportName.BuktiJurnalUmum)
  ...
  Private Sub SetTitle(kode As String)
    ...
    If kode <> Nothing Then
      x = "#" & kode
    End If
    Me.Text = "Jurnal Umum : " & Tipe.ToString & x
  End Sub
  Private Sub SetFormat()
    ...
    txtDebit.Properties.Mask.EditMask = "n2"
    txtKredit.Properties.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.Numeric
    txtKredit.Properties.Mask.EditMask = "n2"
    txtSelisih.Properties.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.Numeric
    txtSelisih.Properties.Mask.EditMask = "n2"
  End Sub
  ...
  Private Sub SetNomor()
    instance.Sumber = sumber.GetObject(Persistent.SumberDataJenis.JU)
  End Sub
  ...
  Private Sub xGridView_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles xGridView.KeyDown
    ...
    If e.KeyCode = Windows.Forms.Keys.F12 Then
      Dim xSumDebit As Double = instance.Detail.Sum(Function(x) x.Debit), xSumKredit As Double = instance.Detail.Sum(Function(y) y.Kredit)
      Dim dtDetail As Persistent.xPJPGlDetail = CType(xGridView.GetRow(xGridView.FocusedRowHandle), Persistent.xPJPGlDetail)
      If xSumDebit > xSumKredit Then
        dtDetail.Kredit = xSumDebit - xSumKredit
      Else
        dtDetail.Debit = xSumKredit - xSumDebit
      End If
    End If
  End Sub
  ...
  Private Sub xGridView_ColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs) Handles xGridView.ColumnWidthChanged
    xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("pjpJUTransaksiGrid.xml"))
  End Sub
  Private Sub xpColTKI_ResolveSession(sender As Object, e As ResolveSessionEventArgs) Handles xpColTKI.ResolveSession
    e.Session = session
  End Sub
  ...
  Private Sub HitungSaldo()
    txtDebit.EditValue = instance.Detail.Sum(Function(x) x.Debit)
    txtKredit.EditValue = instance.Detail.Sum(Function(x) x.Kredit)
    txtSelisih.EditValue = instance.Detail.Sum(Function(x) x.Debit) - instance.Detail.Sum(Function(x) x.Kredit)
  End Sub
End Class