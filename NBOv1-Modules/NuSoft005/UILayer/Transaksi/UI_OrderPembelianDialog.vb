...
Friend Class UI_OrderPembelianDialog
  Private instance As Persistent.OrderPembelian
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.OrderPembelianDetail)
  ...
  Sub New()
    ...
    Persistent.OrderPembelian.AutoSaveOnEndEdit = False
    Persistent.OrderPembelianDetail.AutoSaveOnEndEdit = False
  ...
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.OrderPembelian(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.OrderPembelian)(Convert.ToInt64(IdToEdit))
  ...
  Private Sub DisableControl(xSave As Boolean)
    ...
    btnAmbilPermintaan.Enabled = False
    txtStatus.Properties.ReadOnly = True
    xGridView.OptionsBehavior.Editable = False

    txtTanggal.Enabled = xSave
    txtTanggal.Enabled = xSave
    txtReffNo.Enabled = xSave
    txtLeadTime.Enabled = xSave
    txtFOB.Enabled = xSave
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      For Each item In instance.Detail
        item.Barang.LastPurchase = item.Harga
      Next
  ...
  Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
    If Not e.NewValue Is Nothing Then
      ...
      xGHistory.DataSource = New XPCollection(Of Persistent.OrderPembelian)(session, New BinaryOperator("Vendor", e.NewValue, BinaryOperatorType.Equal))

      Dim fob = CType(CType(e.NewValue, NuSoft001.Persistent.Kontak).FOB, Integer)
      txtFOB.EditValue = CType(fob, Persistent.eTipeFOB)
      txtPajak.EditValue = CType(CType(val.TipePajakPembelian, Integer), Persistent.eTipePajak)
    End If
  End Sub
  ...
  Private Sub xGridView_CellValueChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    If Not e.Value Is Nothing Then
      Dim row As Persistent.OrderPembelianDetail = CType(xGridView.GetRow(e.RowHandle), Persistent.OrderPembelianDetail)
  ...
  Private Sub xGridView_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles xGridView.KeyDown
    If e.KeyCode = Windows.Forms.Keys.Delete Then
      Dim Id As Persistent.OrderPembelianDetail = CType(xGridView.GetRow(xGridView.FocusedRowHandle), Persistent.OrderPembelianDetail)
  ...
  Private Sub colHargaRepo_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles colHargaRepo.ButtonClick
    If e.Button.Kind = ButtonPredefines.Glyph Then
      Dim row = CType(xGridView.GetFocusedRow, Persistent.OrderPembelianDetail)
      Dim frm As New UI_HistoryHarga
      frm.LoadData(session, row.Barang)
      frm.ShowDialog()
    End If
  End Sub
End Class