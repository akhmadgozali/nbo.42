...
Public Class UI_SanYuPembayaran
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.SanYu_PembayaranHutang, pPending As Integer
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    Dim dtInvoice As New List(Of Persistent.SanYu_PenerimaanInvoice)

    pPending = 0
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        proxy = CType(xGridView.GetRow(selectedData(i).Row), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
        item = CType(proxy.OriginalRow, Persistent.SanYu_PembayaranHutang)
        If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Pending Then
          ...
          '	zdtGL.Delete()
          'End If
          Logic.Umum.SaveLog(session, Persistent.TipeLOG.Delete, "Admin", "Pembayaran Hutang: Hapus " & item.Kode)
          item.Delete()
        Else
          pPending += 1
        End If
    ...
    For i = dtInvoice.Count - 1 To 0 Step -1
      ...
      '	mustUpdate = True      'ElseIf dtInvoice(I).NoExim.PO.DariDomestik Then
      '	mustUpdate = True
      'End If

      If mustUpdate Then
        Dim inv = dtInvoice(i)
        Dim dataBayar = New XPQuery(Of Persistent.SanYu_PembayaranHutangDetail)(session).Where(Function(w) w.NoInvoice Is inv)
  ...
  Overrides Function GetToolTipContent(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
    Dim item As Persistent.SanYu_PembayaranHutang
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.SanYu_PembayaranHutang, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
