...
Friend Class UI_NishiPembayaranDialog
  Private instance As Persistent.NishiPembayaranHutang
  Private xdataHutang As XPCollection(Of Persistent.NishiViewInvoice)
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.NishiPembayaranHutangDetail)
  Private defaultRegional As NPO.Modules.ModSys.Regional
  Private setting As NuSoft001.Logic.FinaSetting
  Private settingpurchase As Logic.PurchaseSetting
  Private sumber As Persistent.SumberDataCollection
  Private StatusLama As NuSoft001.Persistent.StatusTransaksiEnum
  Sub New()
    InitializeComponent()
    NoDropIdentityMap = True
    Persistent.NishiPembayaranHutang.AutoSaveOnEndEdit = False
    Persistent.NishiPembayaranHutangDetail.AutoSaveOnEndEdit = False
    Persistent.NishiViewInvoice.AutoSaveOnEndEdit = False
  End Sub
  ...
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.NishiPembayaranHutang(session)
      ...
      ChangeCode()
      txtKontak.Focus()
      StatusLama = NuSoft001.Persistent.StatusTransaksiEnum.Pending
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.NishiPembayaranHutang)(Convert.ToInt64(IdToEdit))
      ...
      StatusLama = instance.StatusTransaksi
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      Dim item As Persistent.NishiPembayaranHutangDetail
      ...
      For pI = 0 To xdataHutang.Count - 1
        If xdataHutang(pI).NilaiBayarCash > 0 Then
          ...
          Else
            item = New Persistent.NishiPembayaranHutangDetail(session)
            item.NoInvoice = xdataHutang(pI).NoInvoice
            item.NilaiBayarCash = xdataHutang(pI).NilaiBayarCash
            item.NilaiBayar = xdataHutang(pI).NilaiBayarCash
            item.Kurs = xdataHutang(pI).Kurs
      ...
      For i = 0 To xdataHutang.Count - 1
        If Session.IsObjectToSave(xdataHutang(i)) Then
          xdataHutang(i).Reload()
        End If
      Next
      instance.Save()
      ...
      For pI = 0 To instance.Detail.Count - 1
        Dim xDataHutang As New XPCollection(Of Persistent.NishiViewInvoice)(session, New BinaryOperator("NoInvoice.Id", instance.Detail(pI).NoInvoice.Id, BinaryOperatorType.Equal))
  ...
  Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
    If Not e.NewValue Is Nothing Then
      ...
      xGHistory.DataSource = New XPCollection(Of Persistent.NishiPembayaranHutang)(session, New BinaryOperator("Vendor", e.NewValue, BinaryOperatorType.Equal))
      ...
      Filter.Add(New InOperator("NoInvoice.Sumber", New Persistent.SumberDataJenis() {Persistent.SumberDataJenis.RI}))
      Filter.Add(New InOperator("NoInvoice.StatusTransaksi", New Persistent.SumberDataJenis() {NuSoft001.Persistent.StatusTransaksiEnum.Approved, NuSoft001.Persistent.StatusTransaksiEnum.InProgress}))
      'Filter.Add(New BinaryOperator("NoInvoice.StatusTransaksi", NuSoft001.Persistent.StatusTransaksiEnum.Approved, BinaryOperatorType.Equal))


      'MsgBox("Filter data : " & GroupOperator.And(Filter).ToString)
      xdataHutang = New XPCollection(Of Persistent.NishiViewInvoice)(session, GroupOperator.And(Filter))
  ...
  Private Sub xGridView_CellValueChanged1(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    Dim row As Persistent.NishiViewInvoice = CType(xGridView.GetRow(e.RowHandle), Persistent.NishiViewInvoice)
