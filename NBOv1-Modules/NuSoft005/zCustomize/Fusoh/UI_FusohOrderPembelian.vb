...
Public Class UI_FusohOrderPembelian
	...
	Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
		Return New UI_FusohOrderPembelianDialog
	End Function
	...
	Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
		Dim item As Persistent.Fusoh_OrderPembelian, pPending As Integer
    ...
    Try
      If kodeDelete <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Hapus, "Data Order Pembelian [PO] ->" & kodeDelete.Substring(1))
        session.CommitChanges()
      End If
    Catch ex As Exception
    End Try
  ...
  Overrides Function GetToolTipTitle(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
	...
	Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
		Dim item As Persistent.Fusoh_OrderPembelian, xGagal As Integer, xMessage As String
		Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    ...
    Try
      If kodeStatus <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Edit, "Data Order Pembelian [PO] " & xTipe & " ->" & kodeStatus.Substring(1))
        session.CommitChanges()
      End If
    Catch ex As Exception
    End Try
  ...
  Private Sub btnHistory_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnHistory.ItemClick
		...
		If Not xNoTransaksi Is Nothing Then
			Fusoh_Logic.Umum.xHistoryData(Me, session, UI_FusohCekData.TipeTransaksi.PO, xNoTransaksi)
		End If
	End Sub
	...
	Private Sub RevisiLength(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnRevisiLength.ItemClick, popupRevisiLength.ItemClick
		If xGridView.IsDataRow(xGridView.FocusedRowHandle) Then
			...
			Dim item = CType(proxy.OriginalRow, Persistent.Fusoh_OrderPembelian)

			Dim frm As New UI_FusohOrderPembelianDialogRevisiLength
			frm.session = session
			frm.LoadingData(item)
			frm.ShowDialog()
		End If
	End Sub
  ...
  Private Sub btnShowLog_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnShowLog.ItemClick
    If xGridView.IsDataRow(xGridView.FocusedRowHandle) Then
      ...
      frm.session = session
      frm.TipeData = Persistent.SumberDataJenis.PO
      Dim proxy = CType(xGridView.GetFocusedRow(), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
      Dim item = CType(proxy.OriginalRow, Persistent.Fusoh_OrderPembelian)
      frm.Kode = item.Kode
      frm.ShowDialog()
