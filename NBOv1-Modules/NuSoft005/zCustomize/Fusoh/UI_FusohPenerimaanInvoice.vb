...
Public Class UI_FusohPenerimaanInvoice
    ...
    Return New UI_FusohPenerimaanInvoiceDialog
  End Function
				...
				If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Pending Then
					If item.NoExim Is Nothing Then
						If Not item.NoOrder Is Nothing Then
							'=============== update status po
							Dim xpGR = New XPQuery(Of Persistent.Fusoh_PenerimaanInvoice)(session).Where(Function(w) w.NoOrder Is item.NoOrder AndAlso Not w Is item)
						...
						End If
					Else
						Dim xpGR = New XPQuery(Of Persistent.Fusoh_PenerimaanInvoice)(session).Where(Function(w) w.NoExim Is item.NoExim AndAlso Not w Is item)
						...
						If item.NoExim.PO.DariDomestik Then
						...
						Else
							...
							item.NoExim.StatusTransaksi = status
						End If
					End If
