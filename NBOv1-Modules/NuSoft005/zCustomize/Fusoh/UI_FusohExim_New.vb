...
Public Class UI_FusohExim_New
	...
	Overrides Sub FirstLoad()
		...
		ifSource.FixedFilterCriteria = New InOperator("Regional", UserInfo.GetUserRegional(session, NamaDatabase))
	...
	Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
		Dim service = New Service_Fusoh.Service_FusohExim(session)
		Dim deleted = New List(Of Persistent_Fusoh.Persistent_FusohExim)

		For Each x In selectedData
			If Not xGridView.IsGroupRow(x.Row) Then
				Dim proxy = CType(xGridView.GetRow(x.Row), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
				deleted.Add(CType(proxy.OriginalRow, Persistent_Fusoh.Persistent_FusohExim))
			End If
		Next
	...
	Overrides Function GetToolTipContent(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
		Dim item As Persistent_Fusoh.Persistent_FusohExim
		...
		Try
			...
			item = CType(proxy.OriginalRow, Persistent_Fusoh.Persistent_FusohExim)
	...
	Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
		Dim item As Persistent_Fusoh.Persistent_FusohExim, xGagal As Integer, xMessage As String
		Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
