...
Friend Class UI_FusohEximDialogSerial
	Private _data As Persistent.Fusoh_EximDetail
	Private _serialData As XPCollection(Of Persistent.Fusoh_EximDetailSerial)

	Sub LoadingData(data As Persistent.Fusoh_EximDetail)
		...
		txtMulaiDari.Properties.MinValue = 1
		txtUrutan1.Properties.MinValue = 1
		_data = data
		xGrid.DataSource = _data.SerialTemp

		_serialData = New XPCollection(Of Persistent.Fusoh_EximDetailSerial)(session)
		Dim namaMillsheet = New XPQuery(Of Persistent.Fusoh_EximDetailSerial)(session).Where(Function(w) Not String.IsNullOrEmpty(w.MillSheet)).GroupBy(Function(g) g.MillSheet).Select(Function(s) s.Key).ToArray
    ...
    txtMillsheet.MaskBox.AutoCompleteCustomSource = xx
		...
		txtStart.Properties.MaxValue = _data.SerialTemp.Count
		txtMulaiDari.Value = 1
		txtSelesai.Value = _data.SerialTemp.Count

		'txtAwalan.Text = "KHMI-22004-1A"
		'txtMillsheet.Text = "KHMI-22004-1A"
	End Sub
	Sub Disable()
	...
	Private Sub btn1_Click(sender As Object, e As EventArgs) Handles btn1.Click
		...
		'	If CekDuplicate(nama, _data.SerialTemp(i - mulaiDari).Id) Then
		'		xerror += nama & vbCrLf
		'	Else
		'		_data.SerialTemp(i - mulaiDari).Serial = nama
		'	End If
		'Next

		For i = txtStart.Value - 1 To _data.SerialTemp.Count - 1
			Dim nama As String = txtAwalan.Text & mulaiDari.ToString(format)
			If mulaiDari <= txtSelesai.Value Then
				If CekDuplicate(nama, _data.SerialTemp(i).Id) Then
					xerror += nama & vbCrLf
				Else
					_data.SerialTemp(i).Serial = nama
				End If
			End If
			mulaiDari = mulaiDari + 1
		Next
	...
	Private Sub btnMillsheet_Click(sender As Object, e As EventArgs) Handles btnMillsheet.Click
		...
		For i = txtUrutan1.Value To txtUrutan2.Value
			_data.SerialTemp(i - 1).MillSheet = txtMillsheet.Text
		Next
		xGrid.EndUpdate()
	End Sub
End Class