...
Friend Class UI_FusohPembayaranDialog
	Private instance As Persistent.Fusoh_PembayaranHutang
	Private xDataHutangDetail As BindingList(Of Persistent.Fusoh_BayarHutangTemp)
	Private isShown As Boolean
	Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
	Private editAssign As Boolean = False
	Private detailToDelete As New List(Of Persistent.Fusoh_PembayaranHutangDetail)
	...
	Overrides Sub InitializeData()
		...
		If Tipe = InputType.Tambah Then
			instance = New Persistent.Fusoh_PembayaranHutang(session)
		...
		Else
			editAssign = True
			xGrid.DataSource = Nothing
			instance = session.GetObjectByKey(Of Persistent.Fusoh_PembayaranHutang)(Convert.ToInt64(IdToEdit))
	...
	Overrides Sub SimpanData()
		...
		Try
			...
			Dim item As Persistent.Fusoh_PembayaranHutangDetail
			...
			For pI = 0 To xDataHutangDetail.Count - 1
				If xDataHutangDetail(pI).NilaiBayarCash > 0 Then
					...
					Else
						item = New Persistent.Fusoh_PembayaranHutangDetail(session)
						item.NoInvoice = xDataHutangDetail(pI).NoInvoice
						item.NilaiBayarCash = xDataHutangDetail(pI).NilaiBayarCash
						item.NilaiBayar = xDataHutangDetail(pI).NilaiBayarCash
						item.Kurs = xDataHutangDetail(pI).Kurs
			...
			For pI = 0 To instance.Detail.Count - 1
				''===== po jasa & costsheet
				'If instance.Detail(pI).NoInvoice.NoExim Is Nothing Then
				'	mustUpdate = True
				'	'===== po domestik
				'ElseIf instance.Detail(pI).NoInvoice.NoExim.PO.DariDomestik Then
				'	mustUpdate = True
				'End If

				If mustUpdate Then
					Dim itemHutang = xDataHutangDetail.ToList.Find(Function(f) f.NoInvoice Is instance.Detail(pI).NoInvoice)
					If Not itemHutang Is Nothing Then
						If itemHutang.SisaHutang = itemHutang.NilaiBayarCash Then
							instance.Detail(pI).NoInvoice.StatusTransaksi = StatusTransaksiEnum.Finish
						Else
							instance.Detail(pI).NoInvoice.StatusTransaksi = StatusTransaksiEnum.InProgress
						End If
					End If
				End If
			Next
    ...
    Try
      If Tipe = InputType.Tambah Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Tambah, "Data Pembayaran Hutang [VP] -> " & instance.Kode)
      Else
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Edit, "Data Pembayaran Hutang [VP] -> " & instance.Kode)
      End If
      session.CommitChanges()
    Catch ex As Exception
    End Try
  ...
  End Sub
	...
	Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
		If Not e.NewValue Is Nothing Then
			...
			'isikan transaksinya
			Dim inv = New XPQuery(Of Persistent.Fusoh_PenerimaanInvoice)(session).Where(Function(w) w.Vendor Is e.NewValue AndAlso
			...
			xDataHutangDetail = New BindingList(Of Persistent.Fusoh_BayarHutangTemp)

			For pI = 0 To inv.Count - 1
				Dim item = New Persistent.Fusoh_BayarHutangTemp()
				item.NoInvoice = inv(pI)
				item.Terbayar = New XPQuery(Of Persistent.Fusoh_PembayaranHutangDetail)(session).Where(Function(w) w.NoInvoice Is item.NoInvoice).Sum(Function(s) s.NilaiBayar)
				item.OK = False
				item.Urutan = pI
	...
	Private Sub xGridView_CellValueChanged1(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
		Dim row As Persistent.Fusoh_BayarHutangTemp = CType(xGridView.GetRow(e.RowHandle), Persistent.Fusoh_BayarHutangTemp)
