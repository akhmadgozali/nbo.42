...
Friend Class UI_FusohPenerimaanBarangDialog
	...
	Overrides Sub InitializeData()
		...
		Else
			...
			txtNoPO.EditValue = instance.Exim
  ...
  End Sub
    ...
    Using splashManager As DevExpress.XtraSplashScreen.SplashScreenManager = New DevExpress.XtraSplashScreen.SplashScreenManager(Me, GetType(x_Wait), True, True)
			Try
				...
				instance.Exim = CType(txtNoPO.EditValue, Persistent.Fusoh_Exim)
        ...
        Dim invoice = inv.Find(Function(f) f.NoOrder Is instance.Exim.PO)
        For i = 0 To instance.Detail.Count - 1
          '======= start 19-03-10 ===== set harga beli
          Dim detGr = instance.Detail(i)
          If invoice Is Nothing Then '-- jika invoice belum diterima, buat harga beli dari harga order
          ...
          Else '-- jika invoice diterima, buat harga beli dari harga invoice
            Dim totalCostSheet = invoice.TotalCost
            Dim totalWeight = invoice.NoExim.TotalWeight
            Dim detInvoice = invoice.Detail.ToList.Find(Function(f) f.PODetail Is detGr.EximDetail.PODetail)
            If (detInvoice Is Nothing) Then
              detGr.Harga = 0
            Else
              'Dim biayaCost = (totalCostSheet / totalWeight) * detInvoice.EximDetail.Weight
              'detGr.Harga = ((detInvoice.DPP / detInvoice.Qty) * detInvoice.Main.Kurs) + (biayaCost / detInvoice.Qty)
        ...
        Next
				...
				If Not instance.Exim Is Nothing Then
					...
					'update status exim jika domestik
					If instance.Exim.PO.DariDomestik Then
						instance.Exim.StatusTransaksi = status
					Else
						'update status invoice jika intl
						Dim findAll = inv.FindAll(Function(f) f.NoExim Is instance.Exim)
    ...
    Dim frm As New RF_FusohVoucher(Persistent.SumberDataJenis.GR, MainClass.reportName.zFusohVoucherGRN)
	...
	Private Sub SetNomor()
		...
		instance.Number = New Logic.NumberClass(session) With {.FormatNomor = "CSIM[Y2]-[M0][X2]"} 'setting.FormatNomor} CSIM YY-MMxx
	End Sub
  ...
  End Sub
	...
	Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
		If Not e.NewValue Is Nothing Then
			...
			If Tipe = InputType.Tambah Then
				...
				listCriteria3.Add(New BinaryOperator("PO.DariDomestik", False))
				listCriteria3.Add(New BinaryOperator("StatusTransaksi", StatusTransaksiEnum.Finish, BinaryOperatorType.Equal))
				listCriteria4.Add(New BinaryOperator("PO.DariDomestik", True))
				listCriteria4.Add(New BinaryOperator("StatusTransaksi", status, BinaryOperatorType.Equal))
			...
			End If
			poDs.DisplayableProperties = "PO.Kode;PO.Tanggal;Kode;This"
			txtNoPO.Properties.DataSource = poDs
	...
	Private Sub txtNoPO_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtNoPO.EditValueChanging
		If Not e.NewValue Is Nothing And editAssign = False Then
			Dim xDataPenerimaan As New XPCollection(Of Persistent.Fusoh_PenerimaanBarangDetail)(session, New BinaryOperator("Main.Exim", e.NewValue, BinaryOperatorType.Equal))
