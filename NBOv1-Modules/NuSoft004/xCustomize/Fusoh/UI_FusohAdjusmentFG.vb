...
Public Class UI_FusohAdjusmentFG
  ...
  Overrides Sub FirstLoad()
    ...
    CreateCriteria = GroupOperator.And(New BinaryOperator("Sumber", Persistent.SumberDataJenis.PS, BinaryOperatorType.Equal), New InOperator("Regional", UserInfo.GetUserRegional(Session, NamaDatabase)))
  ...
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_FusohAdjusmentFGDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    ...
    Try
      If kodeDelete <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Hapus, "Data Adjusment FG ->" & kodeDelete.Substring(1))
        session.CommitChanges()
      End If
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    ...
    Try
      If kodeStatus <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Edit, "Data Adjustment FG " & xTipe & " ->" & kodeStatus.Substring(1))
        session.CommitChanges()
      End If
    Catch ex As Exception
    End Try
