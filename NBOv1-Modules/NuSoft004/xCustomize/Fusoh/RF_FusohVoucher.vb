...
Friend Class RF_FusohVoucher
	...
	Overrides Sub InitializeData()
		Select Case _jenis
			Case Persistent.SumberDataJenis.OP
				...
				txtNoLOT1.Properties.DataSource = x
				txtNoLOT2.Properties.DataSource = x
	...
	Overrides Sub Filter()
		...
		Select Case _jenis
      Case Persistent.SumberDataJenis.OP
        Dim xDataOP As New XPCollection(Of Persistent.Fusoh_OrderProduksi)(_sesi, CreateCriteria2)
        For xi = 0 To xDataOP.Count - 1
          If Me.ReportId = 4101 Then
            xDataOP(xi).PrintCounter += 1
          Else
            xDataOP(xi).PrintCounterPR += 1
          End If
        Next
        _sesi.CommitChanges()
        _dataSource = New XPCollection(Of Persistent.Fusoh_OrderProduksiBahanBaku)(_sesi, CreateCriteria)
        'Case Persistent.SumberDataJenis.SO
  ...
  Private Function CreateCriteria2() As CriteriaOperator
    ...
    If Not txtNoLOT1.EditValue Is Nothing Then
      If txtNoLOT2.EditValue Is Nothing Then
        result.Add(New BinaryOperator("NomorLot", String.Format(format, txtNoBukti1.EditValue), BinaryOperatorType.Like))
      Else
        result.Add(New BetweenOperator("NomorLot", txtNoBukti1.EditValue, txtNoBukti2.EditValue))
      End If
    End If
    If Not txtTgl1.EditValue Is Nothing Then
      If txtTgl2.EditValue Is Nothing Then
