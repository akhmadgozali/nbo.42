...
Public Class UI_FusohOrderProduksi
  ...
  Overrides Sub FirstLoad()
    ...
    Try
      If kodeDelete <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Hapus, "Data Dempyou ->" & kodeDelete.Substring(1))
        session.CommitChanges()
      End If
    ...
    Try
      If kodeStatus <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Edit, "Data Dempyou " & xTipe & " ->" & kodeStatus.Substring(1))
        session.CommitChanges()
      End If
    Catch ex As Exception
    End Try
  ...
  '	Next
  'End Sub
  Private Sub btnPrioritas_ItemClick(sender As Object, e As ItemClickEventArgs) Handles btnPrioritas.ItemClick, btnTidakPrioritas.ItemClick
    Dim xText As String = String.Format("Apakah anda ingin merubah {0} menjadi {1} ?", e.Item.Tag, Me.Text)
    Dim x As Integer() = Nothing
    If allowEdit = False Then
      Utils.Win.MessageBox.Show("Anda tidak mendapatkan hak akses untuk merubah transaksi", "Update Status", "", Utils.Win.MessageBox.Button.OK, Utils.Win.MessageBox.Icon.Informasi)
      Exit Sub
    ...
    If Len(xText) > 0 Then
      If Utils.Win.MessageBox.Show(xText, "Update Prioritas " & Me.Text, "", Utils.Win.MessageBox.Button.YaTidak, Utils.Win.MessageBox.Icon.Pertanyaan) = Utils.Win.MessageBox.DialogResult.Ya Then
        SetPrioritas(note, e.Item.Tag.ToString)
        RefreshData()
      End If
    End If
  End Sub
  Sub SetPrioritas(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.Fusoh_OrderProduksi, xGagal As Integer, xMessage As String
    'Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
