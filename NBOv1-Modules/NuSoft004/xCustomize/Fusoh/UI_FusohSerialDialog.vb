...
Friend Class UI_FusohSerialDialog
	Private _data As Persistent.Fusoh_StockAdjustmentDetail
	Private _serialData As XPCollection(Of Persistent.Fusoh_StockAdjustmentDetailSerial)

	Sub LoadingData(data As Persistent.Fusoh_StockAdjustmentDetail)
		...
		_serialData = New XPCollection(Of Persistent.Fusoh_StockAdjustmentDetailSerial)(session)
		Dim namaMillsheet = New XPQuery(Of Persistent.Fusoh_StockAdjustmentDetailSerial)(session).Where(Function(w) Not String.IsNullOrEmpty(w.MillSheet)).GroupBy(Function(g) g.MillSheet).Select(Function(s) s.Key).ToArray
