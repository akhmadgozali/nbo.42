...
Friend Class UI_PermintaanStockDialog
  Private _JenisPermintaan As Persistent.eJenisPermintaan
  Private instance As Persistent.PermintaanStock
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.PermintaanStockDetail)
  Private defaultRegional As NPO.Modules.ModSys.Regional
  Private setting As NuSoft001.Logic.FinaSetting
  Private settinginventory As Logic.InventorySetting
  Private sumber As Persistent.SumberDataCollection
  Sub New(xJenisPermintaan As Persistent.eJenisPermintaan)
    _JenisPermintaan = xJenisPermintaan
    InitializeComponent()
    NoDropIdentityMap = True
    Persistent.PermintaanStock.AutoSaveOnEndEdit = False
    Persistent.PermintaanStockDetail.AutoSaveOnEndEdit = False
  ...
  Overrides Sub LoadBeforeInitialize()
    ...
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("PBTransaksi.xml")) = True Then
      lytTransaksi.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("PBTransaksi.xml"))
    End If
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("PBTransaksiGrid.xml")) = True Then
      xGridView.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("PBTransaksiGrid.xml"))
    End If
  ...
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.PermintaanStock(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.PermintaanStock)(Convert.ToInt64(IdToEdit))
  ...
  Overrides Sub LoadAfterInitialize()
    SetFormat()
    If Tipe = InputType.Tambah Then
      Me.Text = "Permintaan Stock : Tambah"
    Else
      Me.Text = "Permintaan Stock : Edit " & instance.Kode
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      End If
      instance.Jenis = _JenisPermintaan
    ...
    Dim xSumber As Persistent.SumberDataJenis
    If _JenisPermintaan = Persistent.eJenisPermintaan.PermintaanPembelian Then
      xSumber = Persistent.SumberDataJenis.PB
    Else
      xSumber = Persistent.SumberDataJenis.ST
    End If

    Dim frm As New RF_DataDataTransaksi(xSumber)

    frm.txtNoBukti1.Text = instance.Kode

    Dim message As String = "Apakah anda ingin mencetak bukti {0} dengan nomor " & instance.Kode & " ?"
    Dim reportCode As String = ""
    reportCode = MainClass.GetReport(MainClass.reportName.VoucherPermintaan)
    message = String.Format(message, "Permintaan Barang")
  ...
  Private Sub BindingDataSource()
    ...
    colDivisiRepox.DataSource = New XPCollection(Of NPO.Modules.ModSys.Divisi)(session)
  ...
  Private Sub LoadDefault()
    ...
    txtUraian.EditValue = settinginventory.UraianPermintaan
  End Sub
  ...
  Private Sub xGridView_CellValueChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    If Not e.Value Is Nothing Then
      If e.Column Is colKodeBarang Then
        Dim row As Persistent.PermintaanStockDetail = CType(xGridView.GetRow(e.RowHandle), Persistent.PermintaanStockDetail)
  ...
  Private Sub xGridView_ColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs) Handles xGridView.ColumnWidthChanged
    xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("PBTransaksiGrid.xml"))
  End Sub
  ...
  Private Sub xGridView_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles xGridView.KeyDown
    If e.KeyCode = Windows.Forms.Keys.Delete Then
      Dim Id As Persistent.PermintaanStockDetail = CType(xGridView.GetRow(xGridView.FocusedRowHandle), Persistent.PermintaanStockDetail)
