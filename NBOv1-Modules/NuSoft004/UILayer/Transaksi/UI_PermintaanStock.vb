...
Public Class UI_PermintaanStock
  ...
  Sub New(xJenisPermintaan As Persistent.eJenisPermintaan)
    ...
    UseDbSystem = False
    If _JenisPermintaan = Persistent.eJenisPermintaan.PermintaanMutasi Then
      Me.Text = "Data Permintaan Barang"
    Else
      Me.Text = "Data Permintaan Pembelian"
    End If
  End Sub
  Overrides Sub FirstLoad()
    ...
    CreateCriteria = GroupOperator.And(New BinaryOperator("Jenis", _JenisPermintaan, BinaryOperatorType.Equal), New InOperator("Regional", UserInfo.GetUserRegional(session, NamaDatabase)))
    ...
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("PBData.xml")) = True Then
      xGridView.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("PBData.xml"))
    End If
  End Sub
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_PermintaanStockDialog(_JenisPermintaan)
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.PermintaanStock, pPending As Integer
    ...
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        item = CType(proxy.OriginalRow, Persistent.PermintaanStock)
  ...
  Overrides Function GetToolTipContent(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
    Dim item As Persistent.PermintaanStock
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.PermintaanStock, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
  ...
  Overrides Sub GridViewColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs)
    xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("PBData.xml"))
  End Sub
  ...
  Private Sub btnCetakVoucher_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnCetakVoucher.ItemClick
    ...
    If note.Count > 0 Then
      ...
      message = String.Format(message, "Permintaan Stock")
