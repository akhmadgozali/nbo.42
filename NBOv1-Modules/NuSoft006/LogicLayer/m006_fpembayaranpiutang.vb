...
Namespace Persistent
  ...
  Friend Class PembayaranPiutangDetail
    ...
    Property Main As PembayaranPiutang
      ...
      End Set
    End Property
    <Persistent("p_idinvoice"), Association("fk_pembayaran_invoice")>
  ...
  <Persistent("m06vwinvoice")> Friend Class ViewInvoice
    ...
    Property Catatan As String
    Property Pembayaran As PembayaranPiutangDetail
  End Class
  <NonPersistent> Friend Class NonViewRincianPiutang
    Inherits XPLiteObject
    ...
    <Key> Property Invoice As InvoicePenjualan
    Property JumlahBayar As Double
    ReadOnly Property SisaPiutang As Double
      Get
        Return Invoice.NilaiInvoice - JumlahBayar
      End Get
    End Property
  End Class

  Friend Class DataPiutang
    Property NoInvoice As InvoicePenjualan
    Property Urutan As Int64
    Property Terbayar As Double
    Property NilaiBayarCash As Double
    Property Kurs As Double
...
Namespace Global_Services
  Friend Class InvoicePenjualan_Services
    Friend Shared Function GetRincianPiutang(session As UnitOfWork, perTanggal As Date, regional As NPO.Modules.ModSys.Regional, kontak As NuSoft001.Persistent.Kontak, mataUang As NuSoft001.Persistent.MataUang, tampilkanSaldo0 As Boolean) As List(Of Persistent.NonViewRincianPiutang)
      Dim sqlJumlahBayar = "select sum(d_amount) from m06gdetailpayment det left join m06fmainpayment main on det.p_id=main.p_id where main.d_date <= '{0}' and det.p_idinvoice=c.p_id"
      Dim sql = "select cast(p_id as signed) as Invoice,cast(({1}) as double) as JumlahBayar from m06emaininvoice c where d_date <= '{0}'"
      If Not regional Is Nothing Then
        sql &= " and u_regional = " & regional.Id
      End If
      If Not kontak Is Nothing Then
        sql &= " and f_customer = " & kontak.Id
      End If
      ...
      Dim xdata = session.GetObjectsFromQuery(Of Persistent.NonViewRincianPiutang)(sqlExecute).ToList()
      If tampilkanSaldo0 Then
        Return xdata
      Else
        Return xdata.Where(Function(w) w.SisaPiutang > 0).ToList
      End If
    End Function
  End Class
End Namespace