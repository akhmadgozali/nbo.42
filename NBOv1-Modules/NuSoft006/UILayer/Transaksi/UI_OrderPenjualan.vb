...
Public Class UI_OrderPenjualan
  ...
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_OrderPenjualanDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    ...
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Pending Then
          ...
          '  dtSQ.Add(item.NoPenawaran)
          'End If
          dtSQ.AddRange(item.Detail.Where(Function(w) Not w.PenawaranDetail Is Nothing).Select(Function(s) s.PenawaranDetail.Main).ToList)
          item.Delete()
        Else
          pPending += 1
        End If
    ...
    dtSQ = dtSQ.GroupBy(Function(w) w).Select(Function(s) s.Key).ToList
    For Each itemSQ In dtSQ
      Dim xDataOrder = New XPQuery(Of Persistent.OrderPenjualanDetail)(session).Where(Function(w) w.PenawaranDetail.Main Is itemSQ)
      Dim xQtySQ As Double, xQtySO As Double
      xQtySQ = itemSQ.TotalQty
      xQtySO = xDataOrder.Sum(Function(n) n.Qty)
      If xQtySO = 0 Then
        itemSQ.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Posting
      ElseIf xQtySO < xQtySQ Then
  ...
  Private Sub btnHistorySJ_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnHistorySJ.ItemClick
    Dim so As ReadonlyThreadSafeProxyForObjectFromAnotherThread = CType(xGridView.GetFocusedRow(), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
    If Not so Is Nothing Then
      Dim frm As New UI_HistorySJ
      frm.LoadData(CType(so.OriginalRow, Persistent.OrderPenjualan))
      frm.ShowDialog()
    End If
  End Sub
End Class