...
Public Class UI_DPPenjualan
  ...
  Overrides Sub FirstLoad()
    GetSession()
    Dim a As New NuSoft001.Logic.FinaSetting(GetSession)
    Dim CreateCriteria As CriteriaOperator
    colValas.DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric
    colValas.DisplayFormat.FormatString = a.NumericFormatString
    ...
    colTotalDP.DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric
    colTotalDP.DisplayFormat.FormatString = a.NumericFormatString
    colTotalDP.SummaryItem.DisplayFormat = a.NumericFormatString
    CreateCriteria = GroupOperator.And(New BinaryOperator("Sumber", Persistent.SumberDataJenis.DP, BinaryOperatorType.Equal), New InOperator("Regional", UserInfo.GetUserRegional(session, NamaDatabase)))
    ...
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("DPData.xml")) = True Then
      xGridView.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("DPData.xml"))
    End If
  End Sub
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_DPPenjualanDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.DPPenjualan, pPending As Integer
    ...
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        item = CType(proxy.OriginalRow, Persistent.DPPenjualan)
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.DPPenjualan, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
  ...
  Overrides Sub GridViewColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs)
    xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("DPData.xml"))
  End Sub
  Private Sub btnVoucher_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnVoucher.ItemClick
    ...
    If note.Count > 0 Then
      Dim item As Persistent.DPPenjualan
      Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
