...
Friend Class UI_PenerimaanReturDialog
  Private instance As Persistent.PenerimaanReturPenjualan
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.PenerimaanReturPenjualanDetail)
  ...
  Sub New()
    ...
    Persistent.PenerimaanReturPenjualan.AutoSaveOnEndEdit = False
    Persistent.PenerimaanReturPenjualanDetail.AutoSaveOnEndEdit = False
  ...
  Overrides Sub LoadBeforeInitialize()
    ...
    LoadLayout()
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("RNTransaksi.xml")) = True Then
      lytTransaksi.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("RNTransaksi.xml"))
    End If
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("RNTransaksiGrid.xml")) = True Then
      xGridView.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("RNTransaksiGrid.xml"))
    End If
  End Sub
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.PenerimaanReturPenjualan(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.PenerimaanReturPenjualan)(Convert.ToInt64(IdToEdit))
  ...
  Overrides Sub LoadAfterInitialize()
    SetFormat()
    If Tipe = InputType.Tambah Then
      Me.Text = "Penerimaan Retur [RN]: Tambah Data"
    Else
      Me.Text = "Penerimaan Retur [RN]: Edit Data " & instance.Kode
  ...
  Overrides Sub SimpanData()
    ...
    Catch ex As Exception
      ...
      End If
    End Try
    Dim frm As New RF_DataDataTransaksi(Persistent.SumberDataJenis.RN, MainClass.reportName.VoucherRN)
    frm.txtNoBukti1.EditValue = instance.Kode

    Dim message As String = "Apakah anda ingin mencetak bukti {0} dengan nomor " & instance.Kode & " ?"
    Dim reportCode As String = ""
    reportCode = MainClass.GetReport(MainClass.reportName.VoucherRN)
    message = String.Format(message, "Penerimaan Retur")
  ...
  Private Sub SetNomor()
    instance.Sumber = sumber.GetObject(Persistent.SumberDataJenis.RN)
  ...
  Private Sub xGridView_CellValueChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    If Not e.Value Is Nothing Then
      Dim row As Persistent.PenerimaanReturPenjualanDetail = CType(xGridView.GetRow(e.RowHandle), Persistent.PenerimaanReturPenjualanDetail)
  ...
  Private Sub xGridView_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles xGridView.KeyDown
    If e.KeyCode = Windows.Forms.Keys.Delete Then
      Dim Id As Persistent.PenerimaanReturPenjualanDetail = CType(xGridView.GetRow(xGridView.FocusedRowHandle), Persistent.PenerimaanReturPenjualanDetail)
  ...
  Private Sub xGridView_ColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs) Handles xGridView.ColumnWidthChanged
    xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("RNTransaksiGrid.xml"))
  End Sub
