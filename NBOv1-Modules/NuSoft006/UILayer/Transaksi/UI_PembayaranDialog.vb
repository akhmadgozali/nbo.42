...
Friend Class UI_PembayaranDialog
  Private instance As Persistent.PembayaranPiutang
  Private xdatapiutang As List(Of Persistent.DataPiutang)
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.PembayaranPiutangDetail)
  ...
  Sub New()
    ...
    Persistent.PembayaranPiutang.AutoSaveOnEndEdit = False
    Persistent.PembayaranPiutangDetail.AutoSaveOnEndEdit = False
  ...
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.PembayaranPiutang(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.PembayaranPiutang)(Convert.ToInt64(IdToEdit))
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      Dim item As Persistent.PembayaranPiutangDetail
      Dim pI As Integer, xUrutan As Byte = 0
      For pI = 0 To xdatapiutang.Count - 1
        If xdatapiutang(pI).NilaiBayarCash > 0 Then
          If xdatapiutang(pI).NilaiBayarCash > (xdatapiutang(pI).NoInvoice.NilaiInvoice - xdatapiutang(pI).Terbayar) Then
          ...
          Else
            item = New Persistent.PembayaranPiutangDetail(session)
            item.Urutan = xUrutan
            item.NoInvoice = xdatapiutang(pI).NoInvoice
            item.NilaiBayarCash = xdatapiutang(pI).NilaiBayarCash
            item.NilaiBayar = xdatapiutang(pI).NilaiBayarCash
      ...
      For pI = 0 To instance.Detail.Count - 1
        If instance.Detail(pI).NoInvoice.Total = instance.Detail(pI).NoInvoice.TotalPembayaran Then
  ...
  '    xGHistory.DataSource = New XPCollection(Of Persistent.PembayaranPiutang)(session, New BinaryOperator("Pelanggan", e.NewValue, BinaryOperatorType.Equal))
  ...
  '    xdatapiutang = New XPCollection(Of Persistent.ViewInvoice)(session, GroupOperator.And(Filter))
  ...
  Private Sub txtKontak_EditValueChanging(sender As Object, e As ChangingEventArgs) Handles txtKontak.EditValueChanging
		If Not e.NewValue Is Nothing Then
			...
			Dim xKontak = CType(e.NewValue, NuSoft001.Persistent.Kontak)
					...
					xGHistory.DataSource = New XPCollection(Of Persistent.PembayaranPiutang)(session, New BinaryOperator("Pelanggan", xKontak, BinaryOperatorType.Equal))

					xdatapiutang = New XPQuery(Of Persistent.ViewInvoice)(session).Where(Function(w) w.NoInvoice.Pelanggan Is xKontak).Select(
					Function(s) New Persistent.DataPiutang() With {
					...
					xGrid.DataSource = xdatapiutang.Where(Function(w) w.SisaPiutang > 0).ToList
					hitungsaldo()
			'	Catch ex As Exception
  ...
  Private Sub colBayarRepo_Click(sender As Object, e As EventArgs) Handles colBayarRepo.Click
    ...
    Dim row As Persistent.DataPiutang = CType(xGridView.GetFocusedRow, Persistent.DataPiutang)
  ...
  Private Sub xGridView_CellValueChanged1(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    Dim row As Persistent.DataPiutang = CType(xGridView.GetRow(e.RowHandle), Persistent.DataPiutang)
  ...
  Private Sub xGridView_CustomSummaryCalculate(sender As Object, e As DevExpress.Data.CustomSummaryEventArgs) Handles xGridView.CustomSummaryCalculate
    Dim summaryItem = CType(e.Item, GridSummaryItem)
    Dim summaryTag As Integer = Convert.ToInt32(summaryItem.Tag)

    If e.SummaryProcess = DevExpress.Data.CustomSummaryProcess.Finalize Then
      If e.IsTotalSummary Then
        e.TotalValue = GetSummaryValue(summaryTag)
        e.TotalValueReady = True
      End If
    End If
  End Sub
  Public Function GetSummaryValue(tagId As Integer) As Double
    If xdatapiutang Is Nothing Then Return 0
    Dim xData = xdatapiutang.Where(Function(w) w.OK)
    Select Case tagId
      Case 1 : Return xData.Sum(Function(s) s.NoInvoice.NilaiInvoice)
      Case 2 : Return xData.Sum(Function(s) s.Terbayar)
      Case 3 : Return xData.Sum(Function(s) s.NilaiBayarCash)
      Case 4 : Return xData.Sum(Function(s) s.NoInvoice.TotalFeeAgen)
      Case Else : Return 0
    End Select
  End Function
  ...
  Private Sub btnExportGrid_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnExportGrid.ItemClick
    NuSoft.Core.Win.Component.XtraGrid.ExportExcel(xGrid, session, "Export Data")
  End Sub
End Class