...
Public Class UI_SaldoAwalPiutang
  ...
  Overrides Sub FirstLoad()
    ...
    CreateCriteria = GroupOperator.And(New BinaryOperator("Sumber", Persistent.SumberDataJenis.BR, BinaryOperatorType.Equal), New InOperator("Regional", UserInfo.GetUserRegional(session, NamaDatabase)))
  ...
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_SaldoAwalPiutangDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.InvoicePenjualan, pPending As Integer
  ...
  End Function
	Overrides Sub GridViewColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs)
		xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("BRData.xml"))
	End Sub
				...
				Using dialog = New OpenFileDialog
					...
					Using stream = New FileStream(dialog.FileName, FileMode.Open)
						...
						If Not workSheet Is Nothing Then 'NoTransaksi	Regional	Kontak	Nilai	Uang	Kurs	Valas	Keterangan
							...
							Dim xPenerimaanInvoice As New XPCollection(Of Persistent.InvoicePenjualan)(session)
							For i = 1 To usedRange.RowCount - 1
								...
								If Trim(xNama) <> "" Then
									...
									Dim xItem = New Persistent.InvoicePenjualan(session) With {.Number = New Logic.NumberClass(session) With {.FormatNomor = Setting.FormatNomor}}
									xItem.Gl = New Persistent.GlMain(session) With {.Number = New Logic.NumberClass(session) With {.FormatNomor = Setting.FormatNomor}}
									xItem.Sumber = sumber.GetObject(Persistent.SumberDataJenis.BR)
									xItem.Tanggal = xTglSA
									xItem.Regional = xRegional
									xItem.Kode = cellValueNoTransaksi
									xItem.Pelanggan = xKontak
									xItem.Uang = xUang
									...
									SimpanGL(xItem, Setting, xUang.PiutangDagang, 1, cellValueNilai, 0, cellValueValas, 0, cellValueKurs)
									SimpanGL(xItem, Setting, session.GetObjectByKey(Of NuSoft001.Persistent.Coa)(Convert.ToInt32(Setting.PenampungSaldoAwal)), 2, 0, cellValueNilai, 0, 0, cellValueKurs, "")
	...
	Private Function FindColumnPosition(ws As Worksheet, usedRange As Range, headerName As String) As Integer
		...
		Return -1
	End Function
	Private Sub SimpanGL(instance As Persistent.InvoicePenjualan, Setting As NuSoft001.Logic.FinaSetting, Akun As NuSoft001.Persistent.Coa, Urutan As Byte, Debit As Double, Kredit As Double, DebitValas As Double, KreditValas As Double, Kurs As Double, Optional Catatan As String = "")
