...
Friend Class UI_SaldoAwalPiutangDialog
  Private instance As Persistent.InvoicePenjualan
  Private settingpurchase As Logic.SalesSetting
  ...
  Overrides Sub LoadBeforeInitialize()
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("BRTransaksi.xml")) = True Then
      lytTransaksi.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("BRTransaksi.xml"))
    End If

    settingpurchase = New Logic.SalesSetting(session)
  ...
  Public Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      ...
      ChangeCode()
      txtKeterangan.EditValue = "Saldo Awal Piutang"
      txtNilaiPiutang.EditValue = 0
      txtKurs.EditValue = 1
      ...
      txtKontak.Focus()
    Else
      instance = session.GetObjectByKey(Of Persistent.InvoicePenjualan)(CType(IdToEdit, Int64))
      txtNoTransaksi.EditValue = instance.Kode
      txtTanggal.EditValue = instance.Tanggal
      txtRegional.EditValue = instance.Regional
      txtKontak.EditValue = instance.Pelanggan
      txtNamaKontak.Text = instance.Pelanggan.Nama
      ...
      If instance.Uang.Id = setting.MultiMataUangDefault Then
        txtNilaiPiutang.EditValue = instance.NilaiSaldoAwal
        txtValas.EditValue = 0
      Else
        txtNilaiPiutang.EditValue = instance.NilaiSaldoAwal * instance.Kurs
        txtValas.EditValue = instance.NilaiSaldoAwal
      End If
      txtKeterangan.EditValue = instance.Uraian
  ...
  Overrides Sub LoadAfterInitialize()
    txtNilaiPiutang.Properties.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.Numeric
    txtNilaiPiutang.Properties.Mask.EditMask = setting.NumericFormatToString
  ...
  Public Overrides Sub SimpanData()
    ...
    If instance.Uang.Id = setting.MultiMataUangDefault Then
      instance.NilaiSaldoAwal = CType(txtNilaiPiutang.EditValue, Double)
    Else
      instance.NilaiSaldoAwal = CType(txtValas.EditValue, Double)
    End If
    instance.Uraian = txtKeterangan.Text
    ...
    SimpanGL(CType(txtUang.EditValue, NuSoft001.Persistent.MataUang).PiutangDagang, 1, CType(txtNilaiPiutang.EditValue, Double), 0, CType(txtValas.EditValue, Double), 0)
    SimpanGL(session.GetObjectByKey(Of NuSoft001.Persistent.Coa)(Convert.ToInt32(setting.PenampungSaldoAwal)), 2, 0, txtNilaiPiutang.EditValue, 0, 0, "")
    instance.Save()
  ...
  Private Sub txtUang_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtUang.EditValueChanging
    If Not e.NewValue Is Nothing And editAsign = False Then
      ...
      Else
        ...
        txtValas.EditValue = CDbl(txtNilaiPiutang.EditValue) / CDbl(txtKurs.EditValue)
  ...
  Private Sub txtValas_EditValueChanged(sender As Object, e As EventArgs) Handles txtValas.EditValueChanged
    If editAsign = False Then
      txtNilaiPiutang.EditValue = CDbl(txtValas.EditValue) * CDbl(txtKurs.EditValue)
    End If
  End Sub
  ...
  Private Sub SetNomor()
    instance.Sumber = sumber.GetObject(Persistent.SumberDataJenis.BR)
    instance.Number = New Logic.NumberClass(session) With {.FormatNomor = setting.FormatNomor}
  End Sub
  Private Sub txtNilaiPiutang_EditValueChanged(sender As Object, e As EventArgs) Handles txtNilaiPiutang.EditValueChanged
    If editAsign = False Then
