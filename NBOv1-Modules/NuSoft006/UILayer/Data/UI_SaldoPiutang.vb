...
Public Class UI_SaldoPiutang
  ...
  Overrides Sub FirstLoad()
    ...
    Dim b = From z In xData Group z By z.NoInvoice.Regional, z.NoInvoice.Pelanggan.Id, z.NoInvoice.Pelanggan.Kode, z.NoInvoice.Pelanggan.Nama, z.NoInvoice.Uang Into Group Select Regional, Kode, Nama, MataUang = Uang.Kode, TotalInvoice = Group.Sum(Function(x) x.NoInvoice.NilaiInvoice), TotalBayar = Group.Sum(Function(x) x.Terbayar), SisaSaldo = Group.Sum(Function(x) (x.NoInvoice.NilaiInvoice - x.Terbayar)) Order By Kode
    xGrid.DataSource = b
    If xData.Count > 0 Then
      xGridView.ActiveFilterString = "Not [SisaSaldo] In ('0')"
    End If

    Dim c = From z In xData Group z By z.NoInvoice.Pelanggan.Id, z.NoInvoice.Pelanggan.Kode, z.NoInvoice.Pelanggan.Nama Into Group Select Kode, Nama, SisaSaldo = Group.Sum(Function(x) ((x.NoInvoice.NilaiInvoice - x.Terbayar) * x.NoInvoice.Kurs)) Where SisaSaldo > 0 Order By SisaSaldo Descending
