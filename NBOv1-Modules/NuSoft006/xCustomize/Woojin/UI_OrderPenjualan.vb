...
Public Class zWoojinUI_OrderPenjualan
  ...
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New zWoojinUI_OrderPenjualanDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.zWoojinOrderPenjualan, pPending As Integer
    ...
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        item = CType(proxy.OriginalRow, Persistent.zWoojinOrderPenjualan)
  ...
  Overrides Function GetToolTipContent(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
    Dim item As Persistent.zWoojinOrderPenjualan
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.zWoojinOrderPenjualan, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        ElseIf xTipe = "mengunposting" Then
          ...
          ElseIf item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Closed Then
            'cek dulu apakah sudah ada Invoice atau belum 
            Dim xDtInvoice As New XPCollection(Of Persistent.zWoojinInvoicePenjualanDetail)(session, New BinaryOperator("OrderPenjualan.Main.Id", item.Id, BinaryOperatorType.Equal))
            If xDtInvoice.Count > 0 Then
              xMessage = "Hanya transaksi posting yang bisa dirubah statusnya menjadi pending." & vbCrLf & " Dan yang belum terdapat Invoice yg bisa dirubah menjadi Pending"
              xGagal += 1
            Else
              item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Pending
            End If
