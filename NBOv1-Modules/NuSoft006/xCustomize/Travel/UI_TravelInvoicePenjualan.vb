...
Public Class UI_TravelInvoicePenjualan
	...
	Overrides Sub FirstLoad()
		...
		colSubTotalKurs.DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric
		colSubTotalKurs.DisplayFormat.FormatString = Setting.NumericFormatString
		colSubTotalKurs.SummaryItem.DisplayFormat = Setting.NumericFormatString
    colTotalPembayaran.DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric
    colTotalPembayaran.DisplayFormat.FormatString = Setting.NumericFormatString
    colTotalPembayaran.SummaryItem.DisplayFormat = Setting.NumericFormatString
	...
	Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
		Return New UI_TravelInvoicePenjualanDialog
	End Function
	...
	Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
		...
		For i = 0 To selectedData.Count - 1
			If Not xGridView.IsGroupRow(selectedData(i).Row) Then
				...
				item = CType(proxy.OriginalRow, Travel_Invoice)
				Try
	...
	Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
		Dim item As Travel_Invoice, xGagal As Integer, xMessage As String
		Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
		xGagal = 0
		xMessage = ""
		For i = 0 To selectedData.Count - 1
	...
	Private Sub btnCetak_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnCetakVoucher1.ItemClick, btnCetakVoucher2.ItemClick, btnCetakVoucher3.ItemClick
        ...
        Dim salesSetting As New Logic.SalesSetting(session)
          ...
          Case "Voucher Invoice" : reportCode = MainClass.GetReport(MainClass.reportName.zTravelVoucherIV)
            'Case "Faktur Pajak" : reportCode = MainClass.GetReport(MainClass.reportName.TravelVoucherFP)
          Case Else : reportCode = MainClass.GetReport(MainClass.reportName.zTravelVoucherIV)
        End Select
	...
	Private Sub btnPembayaran_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnPembayaran.ItemClick
		If Not xGridView.GetFocusedRowCellDisplayText(colId) Is Nothing Then
			Dim a As New UI_TravelInvoiceHistory(xGridView.GetFocusedRowCellDisplayText(colId))
			a.Text = "History Pembayaran Invoice - " & xGridView.GetFocusedRowCellDisplayText(colKode)
			a.MenuId = MenuId
