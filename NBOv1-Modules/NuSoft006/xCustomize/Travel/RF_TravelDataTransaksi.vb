...
Friend Class RF_TravelDataTransaksi
  ...
  End Sub
	Overrides Sub InitializeData()
		Select Case _jenis
			...
			Case Persistent.SumberDataJenis.DP
				txtNoBukti1.Properties.DataSource = New XPCollection(Of Travel_DownPayment)(_sesi)
				txtNoBukti2.Properties.DataSource = New XPCollection(Of Travel_DownPayment)(_sesi)
			...
			Case Persistent.SumberDataJenis.IV
        txtNoBukti1.Properties.DataSource = New XPCollection(Of Travel_Invoice)(_sesi)
        txtNoBukti2.Properties.DataSource = New XPCollection(Of Travel_Invoice)(_sesi)
      Case Persistent.SumberDataJenis.CP
        txtNoBukti1.Properties.DataSource = New XPCollection(Of Travel_PembayaranInvoice)(_sesi)
        txtNoBukti2.Properties.DataSource = New XPCollection(Of Travel_PembayaranInvoice)(_sesi)
        'Case Persistent.SumberDataJenis.RN
  ...
  End Sub
	Overrides Sub Filter()
		...
		Select Case _jenis
			...
			Case Persistent.SumberDataJenis.DP
				_dataSource = New XPCollection(Of Travel_DownPayment)(_sesi, CreateCriteriaDP)
			Case Persistent.SumberDataJenis.IV
          ...
          Case Else
            _dataSource = New DevExpress.Xpo.XPCollection(Of Travel_InvoiceDetail)(_sesi, CreateCriteria)
        End Select
			Case Persistent.SumberDataJenis.CP
        _dataSource = New DevExpress.Xpo.XPCollection(Of Travel_PembayaranInvoiceDetail)(_sesi, CreateCriteria)
      Case Persistent.SumberDataJenis.RN
	...
	Private Function CreateCriteriaDP() As CriteriaOperator
		...
		If Not txtNoBukti1.EditValue = Nothing Then
			If txtNoBukti2.EditValue = Nothing Then
				result.Add(New BinaryOperator(NameOf(Travel_DownPayment.Kode), String.Format(format, txtNoBukti1.EditValue), BinaryOperatorType.Like))
			Else
				result.Add(New BetweenOperator(NameOf(Travel_DownPayment.Kode), txtNoBukti1.EditValue, txtNoBukti2.EditValue))
			End If
		End If
		If Not txtTgl1.EditValue Is Nothing Then
			If txtTgl2.EditValue Is Nothing Then
				result.Add(New BinaryOperator(NameOf(Travel_DownPayment.Tanggal), txtTgl1.DateTime, BinaryOperatorType.Equal))
			Else
				result.Add(New BetweenOperator(NameOf(Travel_DownPayment.Tanggal), txtTgl1.DateTime, txtTgl2.DateTime))
			End If
		End If
		If txtKontak.Text <> "" Then
      'result.Add(New BinaryOperator(NameOf(Travel_DownPayment.Pelanggan), String.Format(format, txtKontak.Text), BinaryOperatorType.Like))
      result.Add(New BinaryOperator("Pelanggan", CType(txtKontak.EditValue, NuSoft001.Persistent.Kontak), BinaryOperatorType.Equal))
    End If
    If txtKeterangan.Text <> "" Then
			result.Add(New BinaryOperator(NameOf(Travel_DownPayment.Uraian), String.Format(format, txtKeterangan.Text), BinaryOperatorType.Like))
		End If
