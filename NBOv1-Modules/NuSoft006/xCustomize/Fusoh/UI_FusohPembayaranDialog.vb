...
Friend Class UI_FusohPembayaranDialog
	Private instance As Persistent.Fusoh_PembayaranPiutang
	Private xdatapiutang As XPCollection(Of Persistent.Fusoh_ViewInvoice)
	Private isShown As Boolean
	Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
	Private editAssign As Boolean = False
	Private detailToDelete As New List(Of Persistent.Fusoh_PembayaranPiutangDetail)
	...
	Overrides Sub InitializeData()
		...
		If Tipe = InputType.Tambah Then
			instance = New Persistent.Fusoh_PembayaranPiutang(session)
		...
		Else
			editAssign = True
			xGrid.DataSource = Nothing
			instance = session.GetObjectByKey(Of Persistent.Fusoh_PembayaranPiutang)(Convert.ToInt64(IdToEdit))
	...
	Overrides Sub SimpanData()
		...
		Try
			...
			Dim item As Persistent.Fusoh_PembayaranPiutangDetail
			...
			For pI = 0 To xdatapiutang.Count - 1
				If xdatapiutang(pI).NilaiBayarCash > 0 Then
					...
					Else
						item = New Persistent.Fusoh_PembayaranPiutangDetail(session)
						item.Urutan = xUrutan
						item.NoInvoice = xdatapiutang(pI).NoInvoice
						item.NilaiBayarCash = xdatapiutang(pI).NilaiBayarCash
						item.NilaiBayar = xdatapiutang(pI).NilaiBayarCash
    ...
    Try
      If Tipe = InputType.Tambah Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Tambah, "Data Pembayaran Piutang [CP] -> " & instance.Kode)
      Else
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Edit, "Data Pembayaran Piutang [CP] -> " & instance.Kode)
      End If
      session.CommitChanges()
    Catch ex As Exception
    End Try
  ...
  End Sub
	...
	Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
		...
		If Not e.NewValue Is Nothing Then  '' Edited by sbm 03-01-15
			...
			xGHistory.DataSource = New XPCollection(Of Persistent.Fusoh_PembayaranPiutang)(session, New BinaryOperator("Pelanggan", e.NewValue, BinaryOperatorType.Equal))
			...
			xdatapiutang = New XPCollection(Of Persistent.Fusoh_ViewInvoice)(session, GroupOperator.And(Filter))
	...
	Private Sub xGridView_CellValueChanged1(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
		Dim row As Persistent.Fusoh_ViewInvoice = CType(xGridView.GetRow(e.RowHandle), Persistent.Fusoh_ViewInvoice)
