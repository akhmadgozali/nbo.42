...
Public Class UI_FusohPembayaran
	...
	Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
		Dim item As Persistent.Fusoh_PembayaranPiutang, pPending As Integer
		Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
		Dim dtIV As New List(Of Persistent.Fusoh_InvoicePenjualan)
    Dim kodeDelete = ""
    ...
    pPending = 0
		For i = 0 To selectedData.Count - 1
			If Not xGridView.IsGroupRow(selectedData(i).Row) Then
				...
				item = CType(proxy.OriginalRow, Persistent.Fusoh_PembayaranPiutang)
    ...
    Try
      If kodeDelete <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Hapus, "Data Pembayaran Piutang [CP] ->" & kodeDelete.Substring(1))
        session.CommitChanges()
      End If
    ...
    For i = dtIV.Count - 1 To 0 Step -1
			Dim xpIV As New XPCollection(Of Persistent.Fusoh_PembayaranPiutangDetail)(session, New BinaryOperator("NoInvoice.Id", dtIV(i).Id, BinaryOperatorType.Equal))
	...
	Overrides Function GetToolTipContent(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
		Dim item As Persistent.Fusoh_PembayaranPiutang
		Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
	...
	Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
		Dim item As Persistent.Fusoh_PembayaranPiutang, xGagal As Integer, xMessage As String
		Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
		xGagal = 0
		xMessage = ""
    Dim kodeStatus = ""
    ...
    Try
      If kodeStatus <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Edit, "Data Pembayaran Piutang [CP] " & xTipe & " ->" & kodeStatus.Substring(1))
        session.CommitChanges()
      End If
    Catch ex As Exception
    End Try
  ...
  Private Sub btnShowLog_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnShowLog.ItemClick
    If xGridView.IsDataRow(xGridView.FocusedRowHandle) Then
      ...
      frm.session = session
      frm.TipeData = Persistent.SumberDataJenis.CP
      Dim proxy = CType(xGridView.GetFocusedRow(), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
      Dim item = CType(proxy.OriginalRow, Persistent.Fusoh_PembayaranPiutang)
      frm.Kode = item.Kode
      frm.ShowDialog()
    End If
  End Sub
