...
Public Class UI_Fusoh_SQ
	...
	Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
		Dim item As Persistent.Fusoh_Penawaran, pPending As Integer
    ...
    For i = 0 To selectedData.Count - 1
			If Not xGridView.IsGroupRow(selectedData(i).Row) Then
				...
				item = CType(proxy.OriginalRow, Persistent.Fusoh_Penawaran)
    ...
    Try
      If kodeDelete <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Hapus, "Data Penawaran Penjualan [SQ] ->" & kodeDelete.Substring(1))
        session.CommitChanges()
      End If
	...
	Overrides Function GetToolTipContent(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
		Dim item As Persistent.Fusoh_Penawaran
		Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
		Dim result As String = ""
		Dim xSpace As String, NamaBarang As String, Qty As String ', Harga As String
	...
	Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
		Dim item As Persistent.Fusoh_Penawaran, xGagal As Integer, xMessage As String
		Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
		xGagal = 0
		xMessage = ""
    Dim kodeStatus = ""
    ...
    Try
      If kodeStatus <> "" Then
        DatabaseLog.SaveSimpleLog(session, NPO.DatabaseLog.Action.Edit, "Data Penawaran Penjualan [SQ] " & xTipe & " ->" & kodeStatus.Substring(1))
        session.CommitChanges()
      End If
    Catch ex As Exception
    End Try
  ...
  End Sub
	...
	Private Sub btnCetakVooucher_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnCetakVoucher1.ItemClick, btnCetakVoucher2.ItemClick
		Dim x As Integer() = Nothing
		...
		If note.Count > 0 Then
			...
			If e.Item.Name = btnCetakVoucher1.Name Then
				reportCode = MainClass.GetReport(MainClass.reportName.zFusohVoucherSQ)
			Else e.Item.Name = btnCetakVoucher2.Name
				reportCode = MainClass.GetReport(MainClass.reportName.zFusohVoucherSQ2)
			End If
  ...
  Private Sub btnHistory_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnHistory.ItemClick
    ...
    If Not xNoTransaksi Is Nothing Then
      Fusoh_Logic.Umum.xHistoryData(Me, session, UI_FusohCekData.TipeTransaksi.SQ, xNoTransaksi)
    End If
  End Sub
  ...
  Private Sub btnShowLog_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnShowLog.ItemClick
    If xGridView.IsDataRow(xGridView.FocusedRowHandle) Then
      ...
      frm.session = session
      frm.TipeData = Persistent.SumberDataJenis.SQ
      Dim proxy = CType(xGridView.GetFocusedRow(), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
      Dim item = CType(proxy.OriginalRow, Persistent.Fusoh_Penawaran)
      frm.Kode = item.Kode
      frm.ShowDialog()
    End If
  End Sub
