...
Public Class zRexaUI_MutasiStock
  ...
  Overrides Sub FirstLoad()
    ...
    CreateCriteria = GroupOperator.And(New BinaryOperator("Sumber", Persistent.SumberDataJenis.TS, BinaryOperatorType.Equal), New InOperator("Regional", UserInfo.GetUserRegional(session, NamaDatabase)))
    ifSource.FixedFilterCriteria = CreateCriteria
    btnPosting.Enabled = False 'NuSoft.Core.SSystem.User.GetPrivilegesMenu(MenuId, NamaDatabase).Edit
  ...
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New zRexaUI_MutasiStockDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.zRexaStockMain, pPending As Integer
    ...
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        item = CType(proxy.OriginalRow, Persistent.zRexaStockMain)
        If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Pending Then
          If Not item.Gl Is Nothing Then
  ...
  Overrides Function GetToolTipContent(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
    Dim item As Persistent.zRexaStockMain
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.zRexaStockMain, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        ElseIf xTipe = "mengunposting" Then
          ...
          If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Posting Then
            'cek dulu apakah sudah ada hpp out dari mutasi ini
            Dim xData As New XPCollection(Of Persistent.zRexaStockDetailHPPReff)(session, New BinaryOperator("DetailIN.Main.Id", item.Id, BinaryOperatorType.Equal))
            ...
            Else
              ...
              'delete untuk transaksi keluarnya (agar hpp kembali)
              For pi = item.Detail.Count - 1 To 0 Step -1
                If item.Detail(pi).QtyOut > 0 Then
                  item.Detail(pi).Delete()
                End If
              Next
            End If
          Else
  ...
  End Sub
  Private Sub btnCetak_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnCetakVoucher1.ItemClick, btnCetakVoucher2.ItemClick
    Try
      Dim x As Integer() = Nothing
      If baseView.RowCount < 1 Then
        Exit Sub
      ...
      If note.Count > 0 Then
        ...
        Select Case e.Item.Caption
          Case "Voucher Surat Jalan Masuk" : reportCode = MainClass.GetReport(MainClass.reportName.zRexxaVoucherMutasiMasuk)
          Case "Voucher Surat Jalan Keluar" : reportCode = MainClass.GetReport(MainClass.reportName.zRexxaVoucherMutasiKeluar)
          Case Else : reportCode = MainClass.GetReport(MainClass.reportName.zRexxaVoucherMutasiMasuk)
        End Select
