...
Friend Class zRexaUI_PembayaranDialog
  Private instance As Persistent.zRexaPembayaranPiutang
  Private xdatapiutang As XPCollection(Of Persistent.zRexaViewInvoice)
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.zRexaPembayaranPiutangDetail)
  ...
  Sub New()
    ...
    Persistent.zRexaPembayaranPiutang.AutoSaveOnEndEdit = False
    Persistent.zRexaPembayaranPiutangDetail.AutoSaveOnEndEdit = False
    Persistent.zRexaViewInvoice.AutoSaveOnEndEdit = False
  ...
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.zRexaPembayaranPiutang(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.zRexaPembayaranPiutang)(Convert.ToInt64(IdToEdit))
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      Dim item As Persistent.zRexaPembayaranPiutangDetail
      Dim pI As Integer, xUrutan As Byte = 0, xSisaPiutang As Double = 0
      For pI = 0 To xdatapiutang.Count - 1
        If xdatapiutang(pI).NilaiBayarCash > 0 Then
          xSisaPiutang = (xdatapiutang(pI).NoInvoice.Total - xdatapiutang(pI).Terbayar)
          If xSisaPiutang.ToString("n2") = xdatapiutang(pI).NilaiBayarCash.ToString("n2") Then

          Else
            If xdatapiutang(pI).NilaiBayarCash > xSisaPiutang Then
              Throw New Utils.Exception("Jumlah pembayaran tidak boleh lebih dari sisa piutang." & vbCrLf & _
                                        "No Invoice : " & xdatapiutang(pI).NoInvoice.Kode & vbCrLf & _
                                        "Sisa Piutang : " & (xdatapiutang(pI).NoInvoice.Total - xdatapiutang(pI).Terbayar).ToString("n2") & vbCrLf & _
                                        "Pembayaran : " & xdatapiutang(pI).NilaiBayarCash.ToString("n2") & vbCrLf, -9, "", Me.Text)
              Return
            End If
          End If
          ...
          Else
            item = New Persistent.zRexaPembayaranPiutangDetail(session)
            item.Urutan = xUrutan
            item.NoInvoice = xdatapiutang(pI).NoInvoice
            item.NilaiBayarCash = xdatapiutang(pI).NilaiBayarCash
            item.NilaiBayar = xdatapiutang(pI).NilaiBayarCash
      ...
      'Kas/Bank  (D) 
      i = 1
      SimpanGL(instance.coakasbank, i, instance.Total * instance.Kurs, 0, instance.TotalValas, 0, instance.Kurs)
      i += 1
  ...
  Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
    If Not e.NewValue Is Nothing Then
      ...
      xGHistory.DataSource = New XPCollection(Of Persistent.zRexaPembayaranPiutang)(session, New BinaryOperator("Pelanggan", e.NewValue, BinaryOperatorType.Equal))
      ...
      xdatapiutang = New XPCollection(Of Persistent.zRexaViewInvoice)(session, GroupOperator.And(Filter))
  ...
  Private Sub xGridView_CellValueChanged1(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    Dim row As Persistent.zRexaViewInvoice = CType(xGridView.GetRow(e.RowHandle), Persistent.zRexaViewInvoice)
