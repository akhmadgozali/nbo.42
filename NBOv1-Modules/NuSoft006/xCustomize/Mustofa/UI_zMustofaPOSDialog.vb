...
Friend Class UI_zMustofaPOSDialog
  ...
  Private Sub btnCetakStruk_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnCetakStruk.ItemClick
    ...
    System.Diagnostics.Process.Start("http://" & salesSetting.POSServerStruk & "/struk.php?db=" & NamaDatabase & "&nomor=" & txtNoTransaksi.Text)
  End Sub
  Private Sub txtPelanggan_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles txtPelanggan.ButtonClick
    wbTagihan.Navigate("www.mustofanet.com/billing/cektagihan.php?q=" & txtPelanggan.Text)
  End Sub
  Private Sub BarButtonItem1_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonItem1.ItemClick
    Using dialog = New OpenFileDialog
      ...
      If IO.File.Exists(dialog.FileName) Then
        ...
        Dim xMasterItem = New XPQuery(Of NuSoft004.Persistent.Barang)(session).ToList


        Using MyReader As New FileIO.TextFieldParser(dialog.FileName)
          ...
          While Not MyReader.EndOfData
            Try
              ...
              Dim xBarang = xMasterItem.Find(Function(w) w.Kode = currentRow(0))
              If Not xBarang Is Nothing Then
                Dim item As Persistent.PengirimanBarangDetail
                If instance.Detail.ToList.Exists(Function(m) m.Barang.Kode = currentRow(0)) Then
                  item = instance.Detail.Single(Function(m) m.Barang.Kode = currentRow(0))
                Else
                  item = New Persistent.PengirimanBarangDetail(session)
                  instance.Detail.Add(item)
                End If
                item.Barang = xBarang
                item.Qty = item.Qty + currentRow(1)
                item.Harga = currentRow(2)
                item.Satuan = xBarang.Satuan
              End If
