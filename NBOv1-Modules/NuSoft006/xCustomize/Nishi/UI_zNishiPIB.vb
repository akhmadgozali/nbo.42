...
Public Class UI_zNishiPIB
  ...
  Overrides Sub FirstLoad()
    ...
    CreateCriteria = GroupOperator.And(New BinaryOperator("Sumber", Persistent.SumberDataJenis.PI, BinaryOperatorType.Equal), New InOperator("Regional", UserInfo.GetUserRegional(session, NamaDatabase)))
  ...
  '          item.Detail(j).PengirimanBarang.Main.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Posting
  '        Next
  ...
  '  Dim item As Persistent.InvoicePenjualan, xGagal As Integer, xMessage As String
  '  Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
  '  xGagal = 0
  '  xMessage = ""
  '  For i = 0 To selectedData.Count - 1
  ...
  '        Case "Voucher Invoice" : reportCode = MainClass.GetReport(MainClass.reportName.VoucherIV)
  ...
  '        Case Else : reportCode = MainClass.GetReport(MainClass.reportName.VoucherIV)
  '      End Select
