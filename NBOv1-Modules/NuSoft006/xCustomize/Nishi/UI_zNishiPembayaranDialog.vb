...
Friend Class UI_zNishiPembayaranDialog
  Private instance As Persistent.NishiPembayaranPiutang
  Private xdatapiutang As XPCollection(Of Persistent.NishiViewInvoice)
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.NishiPembayaranPiutangDetail)
  ...
  Sub New()
    ...
    Persistent.NishiPembayaranPiutangDetail.AutoSaveOnEndEdit = False
  ...
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.NishiPembayaranPiutang(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.NishiPembayaranPiutang)(Convert.ToInt64(IdToEdit))
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      Dim item As Persistent.NishiPembayaranPiutangDetail
      ...
      For pI = 0 To xdatapiutang.Count - 1
        If xdatapiutang(pI).NilaiBayarCash > 0 Then
          ...
          Else
            item = New Persistent.NishiPembayaranPiutangDetail(session)
            item.Urutan = xUrutan
            item.NoInvoice = xdatapiutang(pI).NoInvoice
            item.NilaiBayarCash = xdatapiutang(pI).NilaiBayarCash
            item.NilaiBayar = xdatapiutang(pI).NilaiBayarCash
  ...
  Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
    ...
    If Not e.NewValue Is Nothing Then  '' Edited by sbm 03-01-15
      ...
      xGHistory.DataSource = New XPCollection(Of Persistent.NishiPembayaranPiutang)(session, New BinaryOperator("Pelanggan", e.NewValue, BinaryOperatorType.Equal))
      ...
      Filter.Add(New InOperator("NoInvoice.Sumber", New Persistent.SumberDataJenis() {Persistent.SumberDataJenis.IV, Persistent.SumberDataJenis.DN}))

      xdatapiutang = New XPCollection(Of Persistent.NishiViewInvoice)(session, GroupOperator.And(Filter))
