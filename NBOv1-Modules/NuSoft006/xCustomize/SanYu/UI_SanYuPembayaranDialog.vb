...
Friend Class UI_SanYuPembayaranDialog
  Private instance As Persistent.SanYu_PembayaranPiutang
  Private xdatapiutang As XPCollection(Of Persistent.SanYu_ViewInvoice)
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.SanYu_PembayaranPiutangDetail)
  ...
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.SanYu_PembayaranPiutang(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.SanYu_PembayaranPiutang)(Convert.ToInt64(IdToEdit))
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      Dim item As Persistent.SanYu_PembayaranPiutangDetail
      ...
      For pI = 0 To xdatapiutang.Count - 1
        If xdatapiutang(pI).NilaiBayarCash > 0 Then
          ...
          Else
            item = New Persistent.SanYu_PembayaranPiutangDetail(session)
            item.Urutan = xUrutan
            item.NoInvoice = xdatapiutang(pI).NoInvoice
            item.NilaiBayarCash = xdatapiutang(pI).NilaiBayarCash
            item.NilaiBayar = xdatapiutang(pI).NilaiBayarCash
      ...
      Next
      instance.Save()
      Logic.Umum.SaveLog(session, IIf(Tipe = InputType.Tambah, Persistent.TipeLOG.Add, Persistent.TipeLOG.Edit), "Admin", "Pembayaran Penjualan : " & IIf(Tipe = InputType.Tambah, "Tambah", "Edit") & " " & instance.Kode)
      '======= Simpan instance
    ...
    Dim frm As New RF_SanyuVoucher(Persistent.SumberDataJenis.CP, MainClass.reportName.zSanyuVoucherCP)
    frm.txtNoBukti1.EditValue = instance.Kode

    Dim message As String = "Apakah anda ingin mencetak bukti {0} dengan nomor " & instance.Kode & " ?"
    Dim reportCode As String = ""
    reportCode = MainClass.GetReport(MainClass.reportName.zSanyuVoucherCP)
  ...
  Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
    ...
    If Not e.NewValue Is Nothing Then  '' Edited by sbm 03-01-15
      ...
      xGHistory.DataSource = New XPCollection(Of Persistent.SanYu_PembayaranPiutang)(session, New BinaryOperator("Pelanggan", e.NewValue, BinaryOperatorType.Equal))
      ...
      xdatapiutang = New XPCollection(Of Persistent.SanYu_ViewInvoice)(session, GroupOperator.And(Filter))
  ...
  Private Sub xGridView_CellValueChanged1(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    Dim row As Persistent.SanYu_ViewInvoice = CType(xGridView.GetRow(e.RowHandle), Persistent.SanYu_ViewInvoice)
