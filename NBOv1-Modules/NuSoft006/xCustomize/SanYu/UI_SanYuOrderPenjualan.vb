...
Public Class UI_SanYuOrderPenjualan
  ...
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_SanYuOrderPenjualanDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.SanYu_OrderPenjualan, pPending As Integer
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    Dim dtSQ As New List(Of Persistent.Penawaran)

    pPending = 0
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        proxy = CType(xGridView.GetRow(selectedData(i).Row), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
        item = CType(proxy.OriginalRow, Persistent.SanYu_OrderPenjualan)
        If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Pending Then
          Logic.Umum.SaveLog(session, Persistent.TipeLOG.Delete, "Admin", "Order Penjualan : Hapus " & item.Kode)
          item.Delete()
        Else
          pPending += 1
        End If
    ...
    'update status transaksi SQ
    For i = dtSQ.Count - 1 To 0 Step -1
      Dim xpSO As New XPCollection(Of Persistent.OrderPenjualan)(session, New BinaryOperator("NoPenawaran.Id", dtSQ(i).Id, BinaryOperatorType.Equal))
      If xpSO.Count > 0 Then
        dtSQ(i).StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.InProgress
      Else
        dtSQ(i).StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Posting
      End If
    Next
    session.CommitChanges()
    If pPending > 0 Then
  ...
  Overrides Function GetToolTipContent(view As DevExpress.XtraGrid.Views.Grid.GridView, info As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo) As String
    Dim item As Persistent.SanYu_OrderPenjualan
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.SanYu_OrderPenjualan, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        If xTipe = "memposting" Then
          ...
          If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Pending Then
            ...
            Logic.Umum.SaveLog(session, Persistent.TipeLOG.Edit, "Admin", "Order Penjualan : Posting " & item.Kode)
          Else
            xGagal += 1
          End If
        ElseIf xTipe = "mengunposting" Then
          ...
          If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Posting Then
            ...
            Logic.Umum.SaveLog(session, Persistent.TipeLOG.Edit, "Admin", "Order Penjualan : Pending " & item.Kode)
          Else
            xGagal += 1
          End If
        ElseIf xTipe = "mengclosing" Then
          ...
          If Not item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Finish Or item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Rejected Then
            ...
            Logic.Umum.SaveLog(session, Persistent.TipeLOG.Edit, "Admin", "Order Penjualan : Closing " & item.Kode)
          Else
            xGagal += 1
          End If
        End If
  ...
  Private Sub btnHistory_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnHistory.ItemClick
    ...
    End If
  End Sub
  Private Sub btnCetakVoucher1_ItemClick(sender As Object, e As ItemClickEventArgs) Handles btnCetakVoucher1.ItemClick
    Dim x As Integer() = Nothing
    If baseView.RowCount < 1 Then
      Exit Sub
    End If
    ...
    End If
  End Sub
  Private Sub btnProduksi_ItemClick(sender As Object, e As ItemClickEventArgs) Handles btnProduksi.ItemClick
    If Not xGridView.DataSource Is Nothing Then
      Dim KodeBarang As String = xGridView.GetFocusedRowCellDisplayText(colKode)
      Dim frm As New UI_SanYuOrderPenjualanHistory(KodeBarang, session)
      frm.session = session
