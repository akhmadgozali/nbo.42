...
Friend Class UI_zParamaPembayaranDialog
  Private instance As Persistent.zParamaPembayaranPiutang
  Private xdatapiutang As XPCollection(Of Persistent.zParamaViewInvoice)
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.zParamaPembayaranPiutangDetail)
  ...
  Sub New()
    ...
    Persistent.zParamaPembayaranPiutang.AutoSaveOnEndEdit = False
    Persistent.zParamaPembayaranPiutangDetail.AutoSaveOnEndEdit = False
    Persistent.zParamaViewInvoice.AutoSaveOnEndEdit = False
  ...
  Overrides Sub LoadBeforeInitialize()
    ...
    End If
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("CPTransaksiGrid.xml")) = True Then
      xGridView.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("CPTransaksiGrid.xml"))
    End If
  ...
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.zParamaPembayaranPiutang(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.zParamaPembayaranPiutang)(Convert.ToInt64(IdToEdit))
      ...
      txtPPNWapu.EditValue = instance.TotalPPN
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      instance.TotalPPN = CType(txtPPNWapu.EditValue, Double)
      instance.Uraian = txtUraian.EditValue
      instance.KursWapu = txtWapuKurs.EditValue
      'isikan detailnya

      Dim item As Persistent.zParamaPembayaranPiutangDetail
      Dim pI As Byte, SisaPiutang As Double
      For pI = 0 To xdatapiutang.Count - 1
        If xdatapiutang(pI).NilaiBayarCash > 0 Then
          SisaPiutang = (xdatapiutang(pI).NoInvoice.NilaiInvoice - xdatapiutang(pI).Terbayar)
          If xdatapiutang(pI).NilaiBayarCash.ToString("n4") <> SisaPiutang.ToString("n4") Then
            If xdatapiutang(pI).NilaiBayarCash > SisaPiutang Then
              Throw New Utils.Exception("Jumlah pembayaran tidak boleh lebih dari sisa piutang." & vbCrLf & _
                                        "Nomor Invoice : " & xdatapiutang(pI).NoInvoice.Kode & vbCrLf & _
                                        "Nilai Pembayaran : " & xdatapiutang(pI).NilaiBayarCash.ToString("n4") & vbCrLf & _
                                        "Sisa Piutang : " & SisaPiutang.ToString("n4"), -9, "", Me.Text)
              Return
            End If
          End If
          ...
          Else
            item = New Persistent.zParamaPembayaranPiutangDetail(session)
            item.NoInvoice = xdatapiutang(pI).NoInvoice
            item.NilaiBayarCash = xdatapiutang(pI).NilaiBayarCash
            item.NilaiBayar = xdatapiutang(pI).NilaiBayarCash
            item.Kurs = xdatapiutang(pI).Kurs
      ...
      'Kas/Bank  (D) 
      i = 1
      SimpanGL(txtAkun.EditValue, i, txtNilaiKas.EditValue, 0, txtNilaiValas.EditValue, 0, txtKurs.EditValue, "")
      i += 1
      ...
      If instance.TotalDiscount > 0 Then 'Pendapatan (K)
        ...
        i = i + 1
      End If
      If instance.TotalPPN > 0 Then 'PPN (WAPU) (K)
        SimpanGL(session.GetObjectByKey(Of NuSoft001.Persistent.Coa)(Convert.ToInt32(settingsales.CoaPPN)), i, instance.TotalPPN * instance.KursWapu, 0, 0, 0, 1, "")
        i = i + 1
        'isikan selisih kurs nya
        Dim pSelisihKurs As Double = (instance.TotalPPN * instance.Kurs) - (instance.TotalPPN * instance.KursWapu)
        If pSelisihKurs <> 0 Then
          SimpanGL(session.GetObjectByKey(Of NuSoft001.Persistent.Coa)(Convert.ToInt32(setting.MultiMataUangAkunKurs)), i, pSelisihKurs, 0, 0, 0, 1, "Selisih Kurs WAPU")
          i = i + 1
        End If
      End If

      'Piutang (K)
      Dim a = From z In instance.Detail Group z By z.NoInvoice.Uang.PiutangDagang, z.NoInvoice.Kurs Into Group Select PiutangDagang, SubTotal = Group.Sum(Function(x) x.NilaiBayarCash * x.NoInvoice.Kurs), SubTotalValas = Group.Sum(Function(x) x.NilaiBayarCash), Kurs = Kurs
      For Each x In a
      ...
      If instance.SelisihKurs <> 0 Then
        SimpanGL(session.GetObjectByKey(Of NuSoft001.Persistent.Coa)(Convert.ToInt32(setting.MultiMataUangAkunKurs)), i, instance.SelisihKurs, 0, 0, 0, 1, "Selisih Piutang")
        i = i + 1
      End If

      For i = 0 To xdatapiutang.Count - 1
        If session.IsObjectToSave(xdatapiutang(i)) Then
          xdatapiutang(i).Reload()
        End If
      Next
  ...
  Private Sub SetFormat()
    ...
    txtPPNWapu.Properties.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.Numeric
    txtPPNWapu.Properties.Mask.EditMask = setting.NumericFormatToString
    txtWapuKurs.Properties.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.Numeric
    txtWapuKurs.Properties.Mask.EditMask = setting.NumericFormatToString
  End Sub
  Private Sub LoadDefault()
    ...
    txtPPNWapu.EditValue = 0.0
    txtWapuKurs.EditValue = 0.0
  ...
  Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
    'If Not e.NewValue Is Nothing And Tipe = InputType.Tambah Then '' Edited by sbm 03-01-15
    If Not e.NewValue Is Nothing Then  '' Edited by sbm 03-01-15
      ...
      xGHistory.DataSource = New XPCollection(Of Persistent.zParamaPembayaranPiutang)(session, New BinaryOperator("Pelanggan", e.NewValue, BinaryOperatorType.Equal))
      ...
      xdatapiutang = New XPCollection(Of Persistent.zParamaViewInvoice)(session, GroupOperator.And(Filter))

      For pI = 0 To xdatapiutang.Count - 1
        xdatapiutang(pI).OK = False
        xdatapiutang(pI).Urutan = pI
        xdatapiutang(pI).NilaiBayarCash = 0.0 ' xdataHutang(pI).NoInvoice.NilaiInvoice - xdataHutang(pI).Terbayar
        '' Edited by sbm 03-01-15
        '' ColSisa = NoInvoice.NilaiInvoice-Terbayar
  ...
  Private Sub txtAkun_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtAkun.EditValueChanging
    If Not e.NewValue Is Nothing Then
      ...
      Else
        ...
        txtKurs.Properties.ReadOnly = False
      End If
      txtWapuKurs.Text = txtKurs.Text
      txtWapuKurs.Properties.ReadOnly = txtKurs.Properties.ReadOnly
      hitungsaldo()
    End If
  End Sub
  ...
  Private Sub hitungsaldo()
    ...
    If Not xGridView.DataSource Is Nothing Then
      ...
      For pi = 0 To xdatapiutang.Count - 1
        If xdatapiutang(pi).OK = True Then
          saldo = saldo + (xdatapiutang(pi).NilaiBayarCash * xdatapiutang(pi).Kurs)
        End If
      Next
      ...
      If txtPPNWapu.EditValue Is Nothing Then txtPPNWapu.EditValue = 0.0
      ...
      txtNilaiKas.EditValue = CDbl(saldo) + ((CDbl(txtNilaiPendapatan.EditValue) + CDbl(txtNilaiPembulatan.EditValue) - CDbl(txtNilaiAdministrasi.EditValue) - CDbl(txtNilaiPPH.EditValue) - CDbl(txtPPNWapu.EditValue)) * CDbl(txtKurs.EditValue))
    ...
    End If
  End Sub
  Private Sub SimpanGL(Akun As NuSoft001.Persistent.Coa, Urutan As Byte, Debit As Double, Kredit As Double, DebitValas As Double, KreditValas As Double, Kurs As Double, Catatan As String)
    ...
    End If
  End Sub
  Private Sub txtNilai_EditValueChanged(sender As Object, e As EventArgs) Handles txtNilaiAdministrasi.EditValueChanged, txtNilaiPembulatan.EditValueChanged, txtNilaiPendapatan.EditValueChanged, txtNilaiPPH.EditValueChanged, txtPPNWapu.EditValueChanged
    hitungsaldo()
  End Sub
  ...
  Private Sub xGridView_CellValueChanged1(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    Dim row As Persistent.zParamaViewInvoice = CType(xGridView.GetRow(e.RowHandle), Persistent.zParamaViewInvoice)
  ...
  Private Sub xGridView_ColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs) Handles xGridView.ColumnWidthChanged
    xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("CPTransaksiGrid.xml"))
  End Sub
End Class