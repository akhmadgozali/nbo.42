...
Public Class UI_zForwarderStuffing
  ...
  Overrides Sub FirstLoad()
    ...
    iList.Add(Persistent.SumberDataJenis.SF)
    ...
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("ForwarderSFData.xml")) = True Then
      xGridView.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("ForwarderSFData.xml"))
    End If
  End Sub
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_zForwarderStuffingDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.zForwarderStuffing, pPending As Integer
    ...
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        item = CType(proxy.OriginalRow, Persistent.zForwarderStuffing)
        If item.StatusTransaksi = NuSoft001.Persistent.StatusTransaksiEnum.Pending Then
          ...
          If Not zdtGL Is Nothing Then
            zdtGL.Delete()
          End If
          If Not item.GLRealisasi Is Nothing Then
            Dim zdtGLR As Persistent.GlMain = session.FindObject(Of Persistent.GlMain)(New BinaryOperator("Id", item.GLRealisasi.Id, BinaryOperatorType.Equal))
            If Not zdtGLR Is Nothing Then
              zdtGLR.Delete()
            End If
          End If
          item.Delete()
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.zForwarderStuffing, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
  ...
  Private Sub btnVoucher1_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnVoucher1.ItemClick, btnVoucher2.ItemClick, btnVoucher3.ItemClick, btnVoucher4.ItemClick, btnVoucher5.ItemClick
    ...
    If note.Count > 0 Then
      Dim item As Persistent.zForwarderStuffing
      Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
      ...
      Select Case e.Item.Caption
        Case "Voucher Stuffing" : reportCode = MainClass.GetReport(MainClass.reportName.zForwarderStuffing)
  ...
  Overrides Sub GridViewColumnWidthChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.ColumnEventArgs)
    xGridView.SaveLayoutToXml(Logic.Umum.xLokasiLayout("ForwarderSFData.xml"))
  End Sub
End Class