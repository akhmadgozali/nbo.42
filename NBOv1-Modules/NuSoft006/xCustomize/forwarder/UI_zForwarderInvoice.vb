...
Public Class UI_zForwarderInvoice
  ...
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_zForwarderInvoiceDialog2
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.zForwarderInvoice, pPending As Integer
    ...
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        ...
        item = CType(proxy.OriginalRow, Persistent.zForwarderInvoice)
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.zForwarderInvoice, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
    ...
    End If
  End Sub
  Private Sub btnVoucher1_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnVoucher1.ItemClick, btnVoucher2.ItemClick, btnVoucher3.ItemClick, btnVoucher4.ItemClick, btnVoucher5.ItemClick, btnICE2.ItemClick, btnDRY2.ItemClick
    Dim x As Integer() = Nothing
    ...
    If note.Count > 0 Then
      Dim item As Persistent.zForwarderInvoice
      Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread

      proxy = CType(xGridView.GetRow(note(0).Row), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
      item = CType(proxy.OriginalRow, Persistent.zForwarderInvoice)

      Dim frm As New RF_zForwarderDataData(Persistent.SumberDataJenis.IV)
      ...
      Select Case e.Item.Caption
        Case "Invoice" : reportCode = MainClass.GetReport(MainClass.reportName.zForwarderVoucherInvoice)
        Case "Invoice Detail DRY" : reportCode = MainClass.GetReport(MainClass.reportName.zForwarderVoucherInvoiceDDry)
        Case "Invoice Detail DRY 2" : reportCode = MainClass.GetReport(MainClass.reportName.zForwarderVoucherInvoiceDDry2)
        Case "Invoice Detail ICE" : reportCode = MainClass.GetReport(MainClass.reportName.zForwarderVoucherInvoiceDIce)
        Case "Invoice Detail ICE 2" : reportCode = MainClass.GetReport(MainClass.reportName.zForwarderVoucherInvoiceDIce2)
        Case "Faktur Pajak" : reportCode = MainClass.GetReport(MainClass.reportName.zForwarderVoucherFP)
        Case Else : reportCode = MainClass.GetReport(MainClass.reportName.zForwarderVoucherFP)
      End Select
