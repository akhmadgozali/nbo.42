...
Public Class UI_zForwarderPembayaran
  ...
  Overrides Function GetDialogForm() As Core.Win.Forms.InputBase
    Return New UI_zForwarderPembayaranDialog
  End Function
  ...
  Overrides Function HapusData(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData)) As Boolean
    Dim item As Persistent.zForwarderPembayaranPiutang, pPending As Integer
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    Dim dtIV As New List(Of Persistent.zForwarderInvoice)

    pPending = 0
    For i = 0 To selectedData.Count - 1
      If Not xGridView.IsGroupRow(selectedData(i).Row) Then
        proxy = CType(xGridView.GetRow(selectedData(i).Row), ReadonlyThreadSafeProxyForObjectFromAnotherThread)
        item = CType(proxy.OriginalRow, Persistent.zForwarderPembayaranPiutang)
    ...
    For i = dtIV.Count - 1 To 0 Step -1
      Dim xpIV As New XPCollection(Of Persistent.zForwarderPembayaranPiutangDetail)(session, New BinaryOperator("NoInvoice.Id", dtIV(i).Id, BinaryOperatorType.Equal))
  ...
  Sub setstatus(selectedData As System.Collections.Generic.List(Of Core.Win.Forms.GridDeletedData), xTipe As String)
    Dim item As Persistent.zForwarderPembayaranPiutang, xGagal As Integer, xMessage As String
    Dim proxy As ReadonlyThreadSafeProxyForObjectFromAnotherThread
    xGagal = 0
    xMessage = ""
    For i = 0 To selectedData.Count - 1
