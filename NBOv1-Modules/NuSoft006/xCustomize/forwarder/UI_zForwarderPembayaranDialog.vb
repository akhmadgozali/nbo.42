...
Friend Class UI_zForwarderPembayaranDialog
  Private instance As Persistent.zForwarderPembayaranPiutang
  Private xdatapiutang As XPCollection(Of Persistent.zForwarderViewInvoice)
  Private isShown As Boolean
  Private xpSettingRegional As XPCollection(Of NuSoft001.Persistent.SettingRegional)
  Private editAssign As Boolean = False
  Private detailToDelete As New List(Of Persistent.zForwarderPembayaranPiutangDetail)
  ...
  Sub New()
    ...
    Persistent.zForwarderPembayaranPiutang.AutoSaveOnEndEdit = False
    Persistent.zForwarderPembayaranPiutangDetail.AutoSaveOnEndEdit = False
    Persistent.zForwarderViewInvoice.AutoSaveOnEndEdit = False
  ...
  Overrides Sub LoadBeforeInitialize()
    ...
    If My.Computer.FileSystem.FileExists(Logic.Umum.xLokasiLayout("ForwarderCPTransaksi.xml")) = True Then
      lytTransaksi.RestoreLayoutFromXml(Logic.Umum.xLokasiLayout("ForwarderCPTransaksi.xml"))
    End If
  End Sub
  Overrides Sub InitializeData()
    ...
    If Tipe = InputType.Tambah Then
      instance = New Persistent.zForwarderPembayaranPiutang(session)
    ...
    Else
      editAssign = True
      xGrid.DataSource = Nothing
      instance = session.GetObjectByKey(Of Persistent.zForwarderPembayaranPiutang)(Convert.ToInt64(IdToEdit))
  ...
  Overrides Sub SimpanData()
    ...
    Try
      ...
      Dim item As Persistent.zForwarderPembayaranPiutangDetail
      Dim pI As Byte
      For pI = 0 To xdatapiutang.Count - 1
        If xdatapiutang(pI).NilaiBayarCash > 0 Then
          ...
          Else
            item = New Persistent.zForwarderPembayaranPiutangDetail(session)
            item.NoInvoice = xdatapiutang(pI).NoInvoice
            item.NilaiBayarCash = xdatapiutang(pI).NilaiBayarCash
            item.NilaiBayar = xdatapiutang(pI).NilaiBayarCash
            item.Kurs = xdatapiutang(pI).Kurs
    ...
    message2 = String.Format(message2, "Jurnal Voucher")
    Core.Win.Report.DirectExecuteReport(frm2, NamaDatabase, reportCode2, message2, True)


    '======= Konfirmasi Cetak Laporan
    'Dim frm As New RF_DataDataTransaksi(Persistent.SumberDataJenis.CP)
  ...
  Private Sub txtKontak_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles txtKontak.EditValueChanging
    If Not e.NewValue Is Nothing And Tipe = InputType.Tambah Then
      ...
      xGHistory.DataSource = New XPCollection(Of Persistent.zForwarderPembayaranPiutang)(session, New BinaryOperator("Customer", e.NewValue, BinaryOperatorType.Equal))

      'isikan transaksinya
      Dim Filter As New List(Of CriteriaOperator)
      Filter.Add(New BinaryOperator("NoInvoice.Customer", e.NewValue, BinaryOperatorType.Equal))

      xdatapiutang = New XPCollection(Of Persistent.zForwarderViewInvoice)(session, GroupOperator.And(Filter))

      For pI = 0 To xdatapiutang.Count - 1
        xdatapiutang(pI).OK = False
        xdatapiutang(pI).Urutan = pI
        xdatapiutang(pI).NilaiBayarCash = 0 ' xdataHutang(pI).NoInvoice.NilaiInvoice - xdataHutang(pI).Terbayar
      Next
  ...
  Private Sub xGridView_CellValueChanged1(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles xGridView.CellValueChanged
    Dim row As Persistent.zForwarderViewInvoice = CType(xGridView.GetRow(e.RowHandle), Persistent.zForwarderViewInvoice)
