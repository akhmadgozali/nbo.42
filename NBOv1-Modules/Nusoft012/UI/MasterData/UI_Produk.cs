...
namespace NuSoft.NUI.Win.Forms.Modules.NuSoft012.UI.MasterData {
	public partial class UI_Produk : GridInput {
		public UI_Produk() { 
			InitializeComponent();
			ActivateToolTip = false;
			baseView = xGridView;
			KeyField = nameof(Produk.Id);
			useFeedbackSource = true;
			useMDIforDialog = false;
			UseDbSystem = false;
		}

		public override InputBase GetDialogForm() { return new UI_ProdukDialog(); }
				...
				if (!xGridView.IsGroupRow(selectedRows[i])) {
					item = new GridDeletedData() {
						...
						Data = string.Format("{0}",
							xGridView.GetRowCellValue(selectedRows[i], nameof(Produk.Kode)))
					};
					result.Add(item);
				}
			}
		...
		public override bool HapusData(List<GridDeletedData> selectedData) {
			var service = new ProdukService(session);
			List<Produk> deleted = new List<Produk>();

			foreach (var x in selectedData) {
				if (!xGridView.IsGroupRow(x.Row)) {
					deleted.Add((Produk)((ReadonlyThreadSafeProxyForObjectFromAnotherThread)xGridView.GetRow(x.Row)).OriginalRow);
				}
			}
