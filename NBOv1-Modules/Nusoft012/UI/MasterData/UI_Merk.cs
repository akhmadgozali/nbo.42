...
namespace NuSoft.NUI.Win.Forms.Modules.NuSoft012.UI.MasterData {
	public partial class UI_Merk : GridInput {
		public UI_Merk() { 
			InitializeComponent();
			ActivateToolTip = false;
			baseView = xGridView;
			KeyField = nameof(Merk.Id);
			useFeedbackSource = true;
			useMDIforDialog = false;
			UseDbSystem = false;
		}

		public override InputBase GetDialogForm() { return new UI_MerkDialog(); }
				...
				if (!xGridView.IsGroupRow(selectedRows[i])) {
					item = new GridDeletedData() {
						...
						Data = string.Format("{0}",
							xGridView.GetRowCellValue(selectedRows[i], nameof(Merk.Kode)))
					};
					result.Add(item);
				}
			}
		...
		public override bool HapusData(List<GridDeletedData> selectedData) {
			var service = new MerkService(session);
			List<Merk> deleted = new List<Merk>();

			foreach (var x in selectedData) {
				if (!xGridView.IsGroupRow(x.Row)) {
					deleted.Add((Merk)((ReadonlyThreadSafeProxyForObjectFromAnotherThread)xGridView.GetRow(x.Row)).OriginalRow);
				}
			}
