...
namespace NuSoft.NUI.Win.Forms.Modules.NuSoft007.UI.PPh {
	public partial class UI_PPh23 : GridInput {
		public UI_PPh23() { 
			InitializeComponent();
		...
		public override void GridViewRowStyle(object sender, RowStyleEventArgs e) {
			try {
				...
				PPh23 item = (PPh23)((ReadonlyThreadSafeProxyForObjectFromAnotherThread)xGridView.GetRow(e.RowHandle)).OriginalRow;
		...
		private void GridViewSelectionChanged(object sender, SelectionChangedEventArgs e) {
			try {
				...
				var row = (PPh23)(xGridView.GetFocusedRow() as ReadonlyThreadSafeProxyForObjectFromAnotherThread).OriginalRow;
		...
		private void UploadClick(object sender, ItemClickEventArgs e) {
			...
			dialog.Title = "Upload file bukti potong untuk invoice " + row.NoInvoice;
			dialog.ShowDialog();
			if (!string.IsNullOrEmpty(dialog.FileName)) {
				try {
					PPh23Service.UploadBuktiPotong(session, dialog.FileName, row);
		...
		private void ShowImage(object sender, ItemClickEventArgs e) {
			...
			if (row.BuktiPotong != null && row.BuktiPotong.FileBlob != null) {
				...
				frm.Text = "Bukti Potong PPh 23 untuk invoice - " + row.NoInvoice;
